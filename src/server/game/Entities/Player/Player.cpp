...
void Player::GiveLevel(uint8 level)
{
	...
	if (level == 80)
	{
        ...
        std::string subject = "Congratulations for level 80";
        std::string text = "Something to get started.";
		MailDraft draft(subject, text);
		Item* item = Item::CreateItem(40753, 200, this);//50x emblem of valor
		item->SaveToDB(trans);
		draft.AddItem(item);
		if (item)
		{
		...
		draft.AddMoney(5000000);
		draft.SendMailTo(trans, this, MailSender(MAIL_NORMAL, this->GetGUIDLow(), MAIL_STATIONERY_GM));
