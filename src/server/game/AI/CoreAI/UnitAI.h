...
struct TargetSelectorTankPlayerDistancesSpell : public std::unary_function<Unit*, bool>
{
    public:
        TargetSelectorTankPlayerDistancesSpell(Creature* source, bool ignoreTank = false, bool playerOnly = false, float minDistance = 0.0f, float maxDistance = 0.0f, int32 spellId = 0) : _source(source), _ignoreTank(ignoreTank), _playerOnly(playerOnly), _minDistance(minDistance), _maxDistance(maxDistance), _spellId(spellId) { }
    ...
    public:
        ...
        template <class PREDICATE> Unit* SelectTarget(SelectAggroTarget targetType, uint32 position, PREDICATE const& predicate)
        {
			if (me->GetMap()->IsDungeon())
				me->ToCreature()->AI()->DoZoneInCombat();
