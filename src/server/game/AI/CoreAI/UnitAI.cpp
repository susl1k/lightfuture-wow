...
bool TargetSelectorTankPlayerDistancesSpell::operator()(Unit* const unit) const
{
	if (!unit)
		return false;
	...
	if (_playerOnly && (unit->GetTypeId() != TYPEID_PLAYER))
		return false;
	
	if (_source && _maxDistance > 0.0f && !_source->IsWithinCombatRange(unit, _maxDistance))
		return false;
	
	if (_source && _minDistance > 0.0f && _source->IsWithinCombatRange(unit, _minDistance))
		return false;
	
	if (_spellId > 0 && !unit->HasAura(_spellId))
		return false;
		
	if (_spellId < 0 && unit->HasAura(-_spellId))
		return false;
	
	if (_source && _ignoreTank)
		return unit != _source->getVictim();
