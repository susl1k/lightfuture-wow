...
void MotionMaster::MoveSmoothPath(uint32 id, const Movement::PointsArray &points, float velocity, bool fly)
{
    Movement::MoveSplineInit init(*_owner);
...
void MotionMaster::MoveSmoothPath(uint32 id, const Position points[], uint32 pointsCount, float velocity, bool fly)
{
...
void MotionMaster::MoveSmoothPath(uint32 id, uint32 npc_id, float velocity, bool fly)
{
	ScriptPointVector const& path = sScriptSystemMgr->GetPointMoveList(npc_id);
