...
void World::LoadConfigSettings(bool reload)
{
    ...
    sLog->outString("WORLD: VMap support included. LineOfSight: %i, getHeight: %i, indoorCheck: %i", enableLOS, enableHeight, enableIndoor);
...
void World::SetInitialWorldSettings()
{
	...
	sLog->outString("Loading Creature Summon Groups...");
    ...
    sLog->outString("Checking month for brewfest...");
    InitBrewfestMonth();
...
void World::Update(uint32 diff)
{
    ...
    if (m_gameTime > m_NextDailyQuestReset)
    {
		...
		if (sWorld->getWorldState(BREWFEST_MONTH) != timePtr->tm_mon + 1)
			sWorld->setWorldState(BREWFEST_MONTH, timePtr->tm_mon + 1);
