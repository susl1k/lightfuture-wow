...
#define GOSSIP_ITEM_1  "I'm ready for summoning."
...
public:
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 action)
	{
		...
		{
			creature->AI()->DoAction(ACTION_SUMMON);
		}
		player->CLOSE_GOSSIP_MENU();
		return true;
	}
	...
	bool OnGossipHello(Player* player, Creature* creature)
	{
		if (!creature->AI()->GetData(DATA_STAGE))
		{
    ...
    struct npc_sarthisAI : public ScriptedAI
    {
        npc_sarthisAI(Creature* c) : ScriptedAI(c) {}
		...
		void DoAction(int32 const action)
		{
			...
			{
				case ACTION_SUMMON:
					me->SummonCreature(23096+stage, -2483.21f, 4699.29f, 154.5f, 4.73f);
					stage++;
					if (stage==5)
						Reset();
				break;
...
class npc_sarthis_elementals : public CreatureScript
{
public:
    npc_sarthis_elementals() : CreatureScript("npc_sarthis_elementals") { }

    CreatureAI* GetAI(Creature* creature) const
    {
		return new npc_sarthis_elementalsAI(creature);
    }
	
    struct npc_sarthis_elementalsAI : public ScriptedAI
    {
        npc_sarthis_elementalsAI(Creature* c) : ScriptedAI(c) {}
		...
		{
			if (me->isSummon())
				me->ToTempSummon()->GetSummoner()->GetAI()->DoAction(ACTION_SUMMON);
		}
		...
		void UpdateAI(uint32 const diff)
		{
			if (hostileTimer <= diff)
				me->setFaction(16);
			else hostileTimer -= diff;
		}
    };
};
