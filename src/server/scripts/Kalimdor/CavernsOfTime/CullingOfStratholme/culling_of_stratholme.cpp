...
public:
                ...
                case 48:
                    ...
                    if (instance)
                        if (instance->GetData(DATA_INFINITE_EVENT) == SPECIAL && IsHeroic())
                        {
                            ...
                            me->SummonCreature(NPC_INFINITE, 2335.47f, 1262.04f, 132.921f, 1.42079f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 86400);
                        }
                    break;
                case 53:
                    ...
                    switch(uiStep)
                    {
                        ...
                        case 57:
                            if (instance->GetData(uiBossEvent) != DONE)
                            {
                                ...
                                uiWave++;
                                uiWaveWorldState++;
                                instance->DoUpdateWorldState(WORLDSTATE_NUMBER_SCOURGE_WAVES_SHOW_COUNT, uiWaveWorldState);
                            }
                            JumpToNextStep(500);
                            break;
    ...
    {
        npc_arthasAI* pAI = CAST_AI(npc_arthasAI,creature->AI());
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* creature)
    {
        ...
        uint32 crates_event_data = instance->GetData(DATA_CRATES_EVENT);
        uint32 arthas_event_data = instance->GetData(DATA_ARTHAS_EVENT);

        if(crates_event_data == NOT_STARTED && creature->GetEntry() == ENTRY_CHROMIE_PART_1)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_CHROMIE_START_1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);

        if(crates_event_data == IN_PROGRESS && creature->GetEntry() == ENTRY_CHROMIE_PART_1 && !pPlayer->HasItemCount(ITEM_ENTRY_ARCANE_DISRUPTOR,1,true))
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_CHROMIE_DISUPTOR_BACK, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+7);

        if(crates_event_data == DONE && arthas_event_data == NOT_STARTED && creature->GetEntry() == ENTRY_CHROMIE_PART_2)
            ...
            case GOSSIP_ACTION_INFO_DEF+6:
                {
                    if(InstanceScript* instance = creature->GetInstanceScript())
                    {
                        ...
                        if(Creature* pArthas = Creature::GetCreature(*creature, instance->GetData64(DATA_ARTHAS)))
                        {
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_cos_arcane_disruptor_targetAI (creature);
    }
