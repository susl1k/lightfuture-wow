...
class go_ribbon_pole : public GameObjectScript
{
    public:
        ...
        bool OnGossipHello(Player* player, GameObject* go)
        {
            if (player)
			{
                player->CastSpell(player, 29531, true);
				if (Creature* upBunny = go->FindNearestCreature(17066, 30.0f))
					upBunny->CastSpell(upBunny, 46829, true);
				Creature* downBunny = go->FindNearestCreature(2000201, 10.0f);
				if (!downBunny)
					downBunny = go->SummonCreature(2000201, *go);
				downBunny->CastSpell(downBunny, 46842, true);
			}
            return true;
        }
};
