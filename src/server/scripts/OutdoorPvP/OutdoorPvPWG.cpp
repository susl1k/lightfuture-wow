...
bool OutdoorPvPWG::SetupOutdoorPvP()
{
    ...
    QueryResult result = WorldDatabase.PQuery("SELECT guid, id, position_y FROM creature WHERE creature.map=571"
    ...
    else
    {
        do
        {
            ...
            switch(fields[1].GetUInt32())
            {
                ...
                case 31036:
					{
						sLog->outError("hledam npc %d", fields[0].GetInt32());
						Unit* qGiver = ObjectAccessor::GetObjectInWorld(fields[0].GetUInt32(), (Unit*)NULL);
						if (qGiver)
							sLog->outError("nazeleno npc %s", qGiver->GetName());
						m_qgVisibleMap[qGiver] = fields[2].GetFloat() > 2520;
					}
					break;
                case 31101:
                case 31102:
                ...
                case 31151:
					{
						...
						if (qGiver)
							sLog->outError("nazeleno npc %s", qGiver->GetName());
						m_qgVisibleMap[qGiver] = fields[2].GetFloat() > 3230;
					}
					break;*/
                /*case 31051:
    ...
    do
    {
        ...
        if (capturePointEntry)
        {
            ...
            {
                delete workshop;
				sLog->outError("WG: Cannot add capture point!");
                continue;
            }
    ...
    if (!m_gate)
    {
		sLog->outError("WG: Cannot find wintergrasp fortress gate!");
        return false;
    }
    ...
    switch(GetCreatureType(entry))
    {
        ...
        case CREATURE_QUESTGIVER:
        {
			switch (creature->GetEntry())
			{
				...
				case 31054:
				case 31036:
					m_qgVisibleMap[creature] = creature->GetPositionY() > 2520;
					creature->SetVisible(m_qgVisibleMap[creature] == (getDefenderTeam() == TEAM_ALLIANCE));
					break;
				case 31101:
				case 31102:
				case 31106:
				...
				case 31091:
				case 31151:
					m_qgVisibleMap[creature] = creature->GetPositionY() > 3230;
					creature->SetVisible(m_qgVisibleMap[creature] == (getDefenderTeam() == TEAM_ALLIANCE));
					break;
			}
...
void OutdoorPvPWG::UpdateAllWorldObject()
{
	...
	for (std::map<Creature*, bool>::iterator itr = m_qgVisibleMap.begin(); itr != m_qgVisibleMap.end(); ++itr)
		if (itr->first)
			itr->first->SetVisible(itr->second == (getDefenderTeam() == TEAM_ALLIANCE));
    ...
    switch(GetCreatureType(entry))
    {
        case CREATURE_TURRET:
        {
            if (isWarTime())
            {
				...
				if (entry == 28366 && creature->GetPositionX() > 4830.0f && getDefenderTeam() == TEAM_ALLIANCE)
				{
					creature->SetVisible(true);
					creature->SetPhaseMask(1, true);
				}
				else if (entry == 28366 && creature->GetPositionX() < 4830.0f && getDefenderTeam() == TEAM_HORDE)
				{
					creature->SetVisible(true);
					creature->SetPhaseMask(1, true);
				}
				else if (entry == 2000209 && creature->GetPositionX() < 4830.0f && getDefenderTeam() == TEAM_ALLIANCE)
				{
					creature->SetVisible(true);
					creature->SetPhaseMask(1, true);
				}
				else if (entry == 2000209 && creature->GetPositionX() > 4830.0f && getDefenderTeam() == TEAM_HORDE)
				{
					creature->SetVisible(true);
					creature->SetPhaseMask(1, true);
				}
        ...
        case CREATURE_SPECIAL:
        {
            ...
            if (creature->GetAreaId() == 4575)
            {
                switch (entry)
                {
                    ...
                    case 39172://Alliance PvP set quartermaster
                    case 32294://Alliance Stone Keeper Shard quartermaster
                    {
                        if (getDefenderTeam() == TEAM_ALLIANCE)
                        {
                            creature->SetPhaseMask(1, true);
                    ...
                    case 32296://Horde Stone Keeper Shard quartermaster
                    case 39173://Horde PvP set quartermaster
                    {
                        if (getDefenderTeam() == TEAM_ALLIANCE)
                        {
                            creature->SetPhaseMask(2, true);
			...
			else if ((creature->GetAreaId() == 4197 ||
				creature->GetAreaId() == 4539) &&
				(entry == 30740 ||
				entry == 30739))
			{
				_RespawnCreatureIfNeeded(creature, getDefenderTeam() == TEAM_ALLIANCE ? 30740 : 30739);
			}
...
void OutdoorPvPWG::RewardMarkOfHonor(Player *plr, uint32 count)
{
    ...
    {
        sLog->outError("Wintergrasp reward item (Entry %u) not exist in `item_template`.", WG_MARK_OF_HONOR);
        return;
    }
...
void OPvPCapturePointWG::ChangeTeam(TeamId oldTeam)
{
    ...
    if (m_team != TEAM_NEUTRAL)
    {
		...
		if (m_engineer)
		{
			...
			m_engineer->GetCreatureListWithEntryInGrid(guards, m_team == TEAM_ALLIANCE ? 30739 : 30740, 100.0f);
			for (std::list<Creature*>::iterator i = guards.begin(); i != guards.end(); i++)
				_RespawnCreatureIfNeeded(*i, m_team == TEAM_ALLIANCE ? 30740 : 30739);
		}
