...
#define GOSSIP_ITEM_1 "Let's go!"
#define GO_ZULAMAN_GATE 186728
class npc_door_event : public CreatureScript
{
    public:
        npc_door_event() : CreatureScript("npc_door_event") { }

        struct npc_door_eventAI : public ScriptedAI
        {
            npc_door_eventAI(Creature* creature) : ScriptedAI(creature)
            {
            }
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 action)
        {
            ...
            switch (action)
            {
                case GOSSIP_ACTION_INFO_DEF+1:
                    ...
                    {
			player->CLOSE_GOSSIP_MENU();
			if (GameObject* go = creature->FindNearestGameObject(GO_ZULAMAN_GATE, 500))
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new npc_door_eventAI(creature);
        }
};
