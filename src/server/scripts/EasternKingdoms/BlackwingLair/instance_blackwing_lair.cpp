...
public:
    ...
    struct instance_blackwing_lair_InstanceMapScript : public InstanceScript
    {
        ...
        {
			uint32 pred = GetBossState(BOSS_RAZORGORE);
				...
				if (data != EggEvent)
					switch (data)
					{
						...
						case NOT_STARTED:
							...
							for (std::list<Creature*>::iterator ite = razorSummons.begin(); ite != razorSummons.end(); ite++)
								(*ite)->DespawnOrUnsummon();
							razorSummons.clear();
							EggEvent = data;
							EggCount = 0;
							break;
						case SPECIAL:
							if (GetBossState(BOSS_RAZORGORE) != IN_PROGRESS)
								return;
							...
							else if (EggEvent == NOT_STARTED)
							{
								EggEvent = IN_PROGRESS;
								_events.ScheduleEvent(EVENT_RAZOR_SPAWN, 45*IN_MILLISECONDS);
							}
							break;
					}
    ...
    protected:
		...
		std::list<Creature*> razorSummons;
    };
