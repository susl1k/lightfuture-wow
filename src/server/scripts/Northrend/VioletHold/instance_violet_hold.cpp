        ...
        void Update(uint32 diff)
        {
            ...
            if (bActive && uiMainEventPhase == IN_PROGRESS)
            {
                if (uiActivationTimer < diff)
                {
                    ...
                    uiActivationTimer = (uiWaveCount == 6 || uiWaveCount == 12) ? 25000 : 5000;
        ...
        void ProcessEvent(WorldObject* go, uint32 uiEventId)
        {
            switch (uiEventId)
            {
                case EVENT_ACTIVATE_CRYSTAL:
					...
					TempSummon* trigger = go->SummonCreature(NPC_DEFENSE_SYSTEM, ArcaneSphere, TEMPSUMMON_MANUAL_DESPAWN, 0);
					if (!trigger)
						break;
