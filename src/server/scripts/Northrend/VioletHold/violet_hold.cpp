            ...
            if (uiPhase)
            {
                if (uiTimer <= uiDiff)
                {
                    switch (uiPhase)
                    {
                        case 1:
                            if (instance)
                                instance->SetData(DATA_DOOR_INTEGRITY, 100);
    ...
    struct mob_azure_binderAI : public violet_hold_trashAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (me->GetEntry() == CREATURE_AZURE_BINDER_1)
            {
                ...
                {
                    DoCastAOE(DUNGEON_MODE(SPELL_ARCANE_EXPLOSION, H_SPELL_ARCANE_EXPLOSION));
                    uiArcaneExplosionTimer = 5000;
...
public:
    ...
    struct npc_violet_hold_arcane_sphereAI : public ScriptedAI
    {
		...
		void DamageDealt(Unit* who, uint32& damage, DamageEffectType type) override
		{
			damage = 0;
		}
