...
#define GOSSIP_START_EVENT1     "I'm ready to start challenge."  // "I'm ready to start challenge."
#define GOSSIP_START_EVENT2     "I'm ready for the next challenge." // "I'm ready for the next challenge."
#define GOSSIP_START_EVENT3     "Let's face our last challenge." // "Let's face our last challenge."
...
const Position GrandChampionSpawnPosition[3] =
{
	{736.768f, 655.148f, 411.614f, 4.6842f},
	{746.768f, 655.148f, 411.614f, 4.6842f},
	{756.768f, 655.148f, 411.614f, 4.6842f}
};
            ...
            if (GetData(EVENT_INTRO) == IN_PROGRESS)
            {
                switch(events.ExecuteEvent())
                {
                            ...
                            for (uint8 i = 0; i < 3; ++i)
                            {
                                if (Creature* add = me->SummonCreature(GetAddId(boss->GetEntry()), SpawnPosition, TEMPSUMMON_MANUAL_DESPAWN))
                                {
                                    ...
                                    switch(i)
                                    {
                                        ...
                                        case 1: add->GetMotionMaster()->MoveFollow(boss, 2.0f, M_PI * 3 / 4); break;
                                        case 2: add->GetMotionMaster()->MoveFollow(boss, 2.0f, M_PI * 5 / 4); break;
                                    }
                                }
                            }
                        }
                    ...
                    case 16:
                        ...
                        if (Creature* boss3 = me->GetCreature(*me, bossGUID[2]))
                            boss3->GetMotionMaster()->MovePoint(0, FactionChampionPosition()[0]);
            ...
            if (GetData(EVENT_CHAMPIONS) == IN_PROGRESS)
            {
                switch(events.ExecuteEvent())
                {
                    ...
                    case 2:
                        for (uint8 i=0; i<3; i++)
                        {
                            if (Creature* boss = me->SummonCreature(bossEntry[i], GrandChampionSpawnPosition[i]))
                            {
                                ...
                                /*switch(i)
                                {
                                    case 1: boss->GetMotionMaster()->MoveFollow(me->GetCreature(*me, bossGUID[0]), 10.0f, (M_PI / 2)); break;
                                    case 2: boss->GetMotionMaster()->MoveFollow(me->GetCreature(*me, bossGUID[0]), 10.0f, (M_PI / 2 + M_PI)); break;
                                }*/
                            }
                        }
                    ...
                    case 4: // Wait for the death of all of them
                        // If wipe restore the following behaviour
                        if (instance->GetData(BOSS_GRAND_CHAMPIONS) == FAIL)
                        {
                            for (uint8 i=0; i<3; i++)
                            {
                                if (Creature* boss = me->GetCreature(*me, bossGUID[i]))
                ...
                switch(events.ExecuteEvent())
                {
                    ...
                    case 4:
                        if (Creature* blackKinght = me->GetCreature(*me, instance->GetData64(DATA_BLACK_KNIGHT)))
                        {
                            ...
                            blackKinght->AddAura(SPELL_DEATH_RESPITE_INTRO, me);
                        }
                        events.ScheduleEvent(5, 4000);
                        break;
                    case 5:
