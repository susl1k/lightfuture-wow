...
const Position ToCOrbLoc[]=
{
	{588.0f, 183.0f, 395.122986f, 0},//1
	{606.0f, 165.0f, 395.122986f, 0},//2
	{613.0f, 140.0f, 395.122986f, 0},//3
	{606.0f, 115.0f, 395.122986f, 0},//4
	{588.0f, 97.0f, 395.122986f, 0},//5
	{563.0f, 90.0f, 395.122986f, 0},//6
	{538.0f, 97.0f, 395.122986f, 0},//7
	{520.0f, 115.0f, 395.122986f, 0},//8
	{513.0f, 140.0f, 395.122986f, 0},//9
	{520.0f, 165.0f, 395.122986f, 0},//10
	{538.0f, 183.0f, 395.122986f, 0},//11
	{563.0f, 190.0f, 395.122986f, 0},//12
};
...
struct boss_twin_baseAI : public ScriptedAI
{
	...
	void Reset() {
		...
		me->SetCanFly(true); */
		m_uiStage = urand(1,4);
		_events.ScheduleEvent(EVENT_SPECIAL_ABILITY, 45000);
		_events.ScheduleEvent(EVENT_SPIKE, 20000);
		if (IsHeroic())
			_events.ScheduleEvent(EVENT_TOUCH, urand(10, 15)*IN_MILLISECONDS);
		_events.ScheduleEvent(EVENT_BERSERK, (IsHeroic() ? 6*MINUTE*IN_MILLISECONDS : 8*MINUTE*IN_MILLISECONDS));
...
public:
	...
	struct mob_bullet_controllerAI : public Scripted_NoMovementAI
	{
		...
		uint32 m_uiOrbSpawnerTimer;
	
		void Reset()
		{
		m_uiOrbSpawnerTimer=1000;
		}

		void UpdateAI(const uint32 uiDiff)
		{
			UpdateVictim();
		if (m_uiOrbSpawnerTimer < uiDiff) {
			for (uint8 i=0;i<12;i++) {
				me->SummonCreature(urand(0,1)==0 ? NPC_BULLET_LIGHT : NPC_BULLET_DARK, ToCOrbLoc[i].GetPositionX(), ToCOrbLoc[i].GetPositionY(), ToCOrbLoc[i].GetPositionZ(), 0.0f, TEMPSUMMON_MANUAL_DESPAWN);
			}
			m_uiOrbSpawnerTimer=8000;
		}
		else m_uiOrbSpawnerTimer -= uiDiff;
		}
	};
};
