...
class npc_announcer_toc10 : public CreatureScript
{
	public:
		...
		bool OnGossipHello(Player* player, Creature* creature)
		{
			...
			{
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Start Northrend Beasts", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Start Lord Jaraxxus", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Start Faction Champions", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Start Twin Valkyr", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+4);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Start Anub'arak", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+5);
				player->SEND_GOSSIP_MENU(MSG_BEASTS, creature->GetGUID());
			}
			return true;
		}
		...
		bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
		{
			...
			switch (action)
			{
				...
				case GOSSIP_ACTION_INFO_DEF+3:
					...
					if (Creature* jaraxxusDespawn=Unit::GetCreature(*creature, instanceScript->GetData64(NPC_JARAXXUS))) {
						jaraxxusDespawn->SetVisible(false);
					}
					if (Creature* wilfredDespawn=Unit::GetCreature(*creature, instanceScript->GetData64(NPC_FIZZLEBANG))) {
						wilfredDespawn->SetVisible(false);
					}
					break;
				...
				if (m_uiUpdateTimer <= uiDiff)
				{
					switch (m_instance->GetData(TYPE_EVENT))
					{
						...
						case 5080:
							...
							if (Creature* fjola = Unit::GetCreature(*me, m_instance->GetData64(NPC_LIGHTBANE)))
								fjola->SetVisible(false);
							if (Creature* eydis = Unit::GetCreature(*me, m_instance->GetData64(NPC_DARKBANE)))
								eydis->SetVisible(false);
					...
					switch (m_instance->GetData(TYPE_EVENT))
					{
						...
						case 150:
							...
							if (m_instance->GetData(TYPE_BEASTS) != DONE)
							{
								...
								{
									temp->GetMotionMaster()->MovePoint(0, ToCCommonLoc[2].GetPositionX(), ToCCommonLoc[2].GetPositionY(), ToCCommonLoc[2].GetPositionZ(), false);
						...
						case 210:
							...
							{
								m_instance->DoUseDoorOrButton(m_instance->GetData64(GO_MAIN_GATE_DOOR));
								if (Creature* temp = me->SummonCreature(NPC_DREADSCALE, ToCSpawnLoc[0].GetPositionX(), ToCSpawnLoc[0].GetPositionY(), ToCSpawnLoc[0].GetPositionZ(), 5, TEMPSUMMON_MANUAL_DESPAWN))
								{
									temp->GetMotionMaster()->MovePoint(0, ToCCommonLoc[8].GetPositionX(), ToCCommonLoc[8].GetPositionY(), ToCCommonLoc[8].GetPositionZ(), false);
									temp->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC);
									temp->SetReactState(REACT_PASSIVE);
								}
								if (Creature* temp = me->SummonCreature(NPC_ACIDMAW, ToCCommonLoc[9].GetPositionX(), ToCCommonLoc[9].GetPositionY(), ToCCommonLoc[9].GetPositionZ(), 5, TEMPSUMMON_MANUAL_DESPAWN))
								{
									...
									temp->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_SELECTABLE);
									temp->SetReactState(REACT_PASSIVE);
								}
							}
							m_uiUpdateTimer = 5000;
						...
						case 310:
							if (m_instance->GetData(TYPE_BEASTS) != DONE)
							{
								...
								{
									temp->GetMotionMaster()->MoveJump(ToCCommonLoc[1].GetPositionX(), ToCCommonLoc[1].GetPositionY(), ToCCommonLoc[1].GetPositionZ(), 40.0f, 35.0f, 2);
									temp->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_IMMUNE_TO_PC);
									temp->SetReactState(REACT_PASSIVE);
								}
							}
						...
						case 4015:
							...
							{
								temp->GetMotionMaster()->MovePoint(1, ToCCommonLoc[8].GetPositionX(), ToCCommonLoc[8].GetPositionY(), ToCCommonLoc[8].GetPositionZ(), false);
								temp->SetVisible(true);
							}
							if (Creature* temp = Unit::GetCreature((*me), m_instance->GetData64(NPC_DARKBANE)))
							{
								temp->GetMotionMaster()->MovePoint(1, ToCCommonLoc[9].GetPositionX(), ToCCommonLoc[9].GetPositionY(), ToCCommonLoc[9].GetPositionZ(), false);
								temp->SetVisible(true);
							}
							m_uiUpdateTimer = 5000;
