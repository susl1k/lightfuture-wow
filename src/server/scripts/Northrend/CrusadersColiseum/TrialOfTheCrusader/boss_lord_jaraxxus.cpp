...
enum Events {
	...
	EVENT_INCENERATE_FLESH =3,
	EVENT_NETHER_POWER     =4,
	EVENT_LEGION_FLAME     =5,
	EVENT_SUMMON_INFERNAL  =6,
	EVENT_SUMMON_PORTAL    =7,
	EVENT_BERSERK          =8,
};
...
public:
	...
	struct boss_jaraxxusAI : public ScriptedAI
	{
		...
		void Reset()
		{
			...
			_events.ScheduleEvent(EVENT_FIREBALL, 18000);
			_events.ScheduleEvent(EVENT_FEL_LIGHTNING, urand(10000, 15000));
			_events.ScheduleEvent(EVENT_INCENERATE_FLESH, 14000);
			_events.ScheduleEvent(EVENT_NETHER_POWER, 42000);
			_events.ScheduleEvent(EVENT_LEGION_FLAME, 20000);
			_events.ScheduleEvent(EVENT_SUMMON_INFERNAL, 80000);
			_events.ScheduleEvent(EVENT_SUMMON_PORTAL, 20000);
		...
		void JustDied(Unit* /*killer*/)
		{
			...
			m_instance->DoRemoveAurasDueToSpellOnPlayers(sSpellMgr->GetSpellIdForDifficulty(SPELL_INCINERATE_FLESH, me));
			m_instance->DoRemoveAurasDueToSpellOnPlayers(sSpellMgr->GetSpellIdForDifficulty(SPELL_LEGION_FLAME, me));
			m_instance->DoRemoveAurasDueToSpellOnPlayers(sSpellMgr->GetSpellIdForDifficulty(SPELL_FEL_FIREBALL, me));
		}
			...
			while (uint32 eventId = _events.ExecuteEvent()) {
				switch (eventId) {
					case EVENT_FIREBALL:
						...
						_events.ScheduleEvent(EVENT_FIREBALL, urand(10000,14000));
					break;
					case EVENT_FEL_LIGHTNING:
						...
						_events.ScheduleEvent(EVENT_FEL_LIGHTNING, urand(10000, 15000));
					break;
					case EVENT_INCENERATE_FLESH:
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true))
						{
							DoScriptText(EMOTE_INCINERATE, me, target);
							DoScriptText(SAY_INCINERATE, me);
							me->AddAura(SPELL_INCINERATE_FLESH, target);
						}
						_events.ScheduleEvent(EVENT_INCENERATE_FLESH, 23000);
					break;
...
public:
	...
	struct mob_infernal_volcanoAI : public Scripted_NoMovementAI
	{
		...
		me->CastSpell(me, SPELL_INFERNAL_ERUPTION_EFFECT);
...
public:
	...
	struct mob_fel_infernalAI : public ScriptedAI
	{
		...
		void Reset()
		{
			m_uiFelStreakTimer = urand(25,30)*IN_MILLISECONDS;
			me->SetInCombatWithZone();
		}
...
public:
	...
	struct mob_nether_portalAI : public ScriptedAI
	{
		...
		{
			//me->CastSpell(me, SPELL_WILFRED_PORTAL);
			me->CastSpell(me, SPELL_NETHER_PORTAL_EFFECT);
		}
...
public:
	...
	struct mob_mistress_of_painAI : public ScriptedAI
	{
		...
		void Reset()
		{
			m_uiShivanSlashTimer = urand(25,30)*IN_MILLISECONDS;
			m_uiSpinningStrikeTimer = urand(25,30)*IN_MILLISECONDS;
