...
enum Achievement
{
	...
	ACHIEVEMENT_SHOCKING_N = 2178, 
	ACHIEVEMENT_SHOCKING_H = 2179,
};
...
public:
	...
	struct boss_thaddiusAI : public BossAI
	{
		...
		void JustDied(Unit* /*killer*/)
		{
			...
			if (shocking)
				instance->DoCompleteAchievement(RAID_MODE(ACHIEVEMENT_SHOCKING_N, ACHIEVEMENT_SHOCKING_H));
		}
					...
					if (instance)
						if (Creature* pStalagg = me->GetCreature(*me, instance->GetData64(DATA_STALAGG)))
						{
							if (!checkStalaggAlive)
								...
								pStalagg->SetHealth(pStalagg->GetMaxHealth());
						}
						...
						if (Creature* pFeugen = me->GetCreature(*me, instance->GetData64(DATA_FEUGEN)))
						{
							if (checkStalaggAlive)
								...
								pFeugen->SetHealth(pFeugen->GetMaxHealth());
						}
				}
			}
			...
			if (magneticPullTimer <= uiDiff)
			{
				if (Creature* pFeugen = me->GetCreature(*me, instance->GetData64(DATA_FEUGEN)))
				{
					...
					if (pFeugenVictim && pStalaggVictim)
					{
						...
						me->AddThreat(pFeugenVictim, pFeugen->getThreatManager().getThreat(pFeugenVictim));
						pFeugen->getThreatManager().modifyThreatPercent(pFeugenVictim, -100);
						pFeugenVictim->JumpTo(me, 0.3f);
						
						pFeugen->AddThreat(pStalaggVictim, me->getThreatManager().getThreat(pStalaggVictim));
...
{
	public:
		RemoveFromList(uint32 triggeringSpellId): _triggeringSpellId(triggeringSpellId) {}
	
		bool operator() (Unit* const unit) const
		{
			return unit->HasAura(_triggeringSpellId);
		}
	...
	public:
		...
		class spell_thaddius_pos_neg_charge_SpellScript : public SpellScript
		{
			...
			void HandleTargets(std::list<Unit*>& targetList)
			{
				...
				uint32 triggeringSpellId = (spellId == SPELL_POSITIVE_CHARGE) ? SPELL_POSITIVE_POLARITY : SPELL_NEGATIVE_POLARITY;
				std::list<Unit*> list = targetList;
				
				uint8 count = 0;
				for (std::list<Unit*>::iterator ihit = targetList.begin(); ihit != targetList.end(); ++ihit)
					if (Player* target = (*ihit)->ToPlayer())
						if (target->HasAura(triggeringSpellId))
							++count;
				...
				if (count)
				{
					...
					{
						GetCaster()->SetAuraStack(SPELL_POSITIVE_CHARGE_STACK, GetCaster(), count);
						GetCaster()->RemoveAurasDueToSpell(SPELL_NEGATIVE_CHARGE_STACK);
					}
					else
					{
						GetCaster()->SetAuraStack(SPELL_NEGATIVE_CHARGE_STACK, GetCaster(), count);
						GetCaster()->RemoveAurasDueToSpell(SPELL_POSITIVE_CHARGE_STACK);
					}
				}
				
				list.remove_if(RemoveFromList(triggeringSpellId));
