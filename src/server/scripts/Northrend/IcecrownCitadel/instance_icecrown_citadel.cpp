...
{
	{NPC_INFILTRATOR_MINCHAR,         1}, // Deprogramming
	{NPC_KOR_KRON_LIEUTENANT,         2}, // Securing the Ramparts
	{NPC_ROTTING_FROST_GIANT_10,      2}, // Securing the Ramparts
	{NPC_ROTTING_FROST_GIANT_25,      2}, // Securing the Ramparts
	{NPC_ALCHEMIST_ADRIANNA,          3}, // Residue Rendezvous
	{NPC_ALRIN_THE_AGILE,             4}, // Blood Quickening
	{NPC_INFILTRATOR_MINCHAR_BQ,      4}, // Blood Quickening
	{NPC_MINCHAR_BEAM_STALKER,        4}, // Blood Quickening
	{NPC_VALITHRIA_DREAMWALKER_QUEST, 5}, // Respite for a Tormented Soul
};
	...
	public:
		...
		struct instance_icecrown_citadel_InstanceMapScript : public InstanceScript
		{
			instance_icecrown_citadel_InstanceMapScript(InstanceMap* map) : InstanceScript(map)
			{
				...
				SindragosaPortEligible = NOT_STARTED;
			...
			void OnCreatureCreate(Creature* creature)
			{
				...
				switch (creature->GetEntry())
				{
					...
					case NPC_GB_KORKRON_BATTLE_MAGE:
						if (GetBossState(DATA_GUNSHIP_EVENT) == IN_PROGRESS)
				...
				switch (entry)
				{
					...
					case NPC_VALITHRIA_DREAMWALKER_QUEST:
					{
						for (uint8 questIndex = 0; questIndex < WeeklyNPCs; ++questIndex)
						{
							...
							{
								if (sWorld->getWorldState(instance->Is25ManRaid() ? WS_ICC_WEEKLY_25 : WS_ICC_WEEKLY_10) != WeeklyQuestData[questIndex].worldState)
									entry = 0;
...
// DENTRO DE ESTA CREATE FALTAN GOS PARA GUNSHIP, DEPENDIENDO LO Q SE QUIERA HACER
			...
			uint32 GetData(uint32 type)
			{
				switch (type)
				{
					...
					case DATA_SINDRAGOSA_PORT_ELIGIBLE:
						return SindragosaPortEligible;
...
// les aparecemos algun npc o alguna cosa estilo wowrean.
					...
					case DATA_BLOOD_QUEEN_LANA_THEL:
						...
						break;
					case DATA_VALITHRIA_DREAMWALKER:
						if (state == DONE && sWorld->getWorldState(instance->Is25ManRaid() ? WS_ICC_WEEKLY_25 : WS_ICC_WEEKLY_10) == WeeklyQuestData[8].worldState)
			...
			void SetData(uint32 type, uint32 data)
			{
				switch (type)
				{
					...
					case DATA_BLOOD_QUICKENING_STATE:
					{
						...
						if (sWorld->getWorldState(instance->Is25ManRaid() ? WS_ICC_WEEKLY_25 : WS_ICC_WEEKLY_10) != WeeklyQuestData[5].worldState)
							break;
