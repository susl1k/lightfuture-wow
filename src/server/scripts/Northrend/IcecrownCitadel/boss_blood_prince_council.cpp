	...
	public:
		...
		struct boss_prince_kelesethAI : public BossAI
		{
			...
			void EnterCombat(Unit* /*who*/)
			{
				...
				events.ScheduleEvent(EVENT_SHADOW_RESONANCE, 10000);
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch (eventId)
					{
						...
						case EVENT_SHADOW_RESONANCE:
						{
							...
							destX = float(rand_norm()) * 66.0f + 4599.0f;
							if (destX > 4630.0f)
								destY = float(rand_norm()) * 94.0f + 2722.0f;
							else
								destY = float(rand_norm()) * 55.0f + 2740.0f;
							destZ = 366.0f;
							me->SummonCreature(NPC_DARK_NUCLEUS, destX, destY, destZ, 0.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 1000);
		...
		struct boss_prince_taldaramAI : public BossAI
		{
			...
			void JustSummoned(Creature* summon)
			{
				...
				if (target)
					...
					summon->CastSpell(target, SPELL_BALL_OF_FLAMES, true);
					summon->AI()->DoAction(ACTION_FLAME_BALL_CHASE);
			}
	...
	public:
		...
		struct boss_prince_valanarAI : public BossAI
		{
			...
			void EnterCombat(Unit* /*who*/)
			{
				...
				events.ScheduleEvent(EVENT_BERSERK, 600000);
				events.ScheduleEvent(EVENT_KINETIC_BOMB, urand(38000, 40000));
				events.ScheduleEvent(EVENT_SHOCK_VORTEX, urand(18000, 20000));
				if (IsHeroic())
					me->AddAura(SPELL_SHADOW_PRISON, me);
			}
			...
			void JustSummoned(Creature* summon)
			{
				switch (summon->GetEntry())
				{
					...
					case NPC_KINETIC_BOMB:
					{
						...
						float ground_Z = summon->GetMap()->GetHeight(x, y, z, true, 500.0f) - 2.0f;
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch (eventId)
					{
						...
						case EVENT_KINETIC_BOMB:
							...
							{
								me->SummonCreature(NPC_KINETIC_BOMB, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ()+20.0f, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 60000);
	...
	public:
		...
		struct npc_ball_of_flameAI : public ScriptedAI
		{
			...
			void DoAction(int32 const action)
			{
				...
				{
					chaseTimer = 1000;
					me->GetMotionMaster()->MovePoint(0, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 3.0f);
			...
			void UpdateAI(uint32 const diff)
			{
				if (_chaseGUID && !exploded)
					if (Unit* target = Unit::GetUnit(*me, _chaseGUID))
						if (chaseTimer)
						{
							if (me->IsWithinDist2d(target, 3.0f))
							{
							...
							if (chaseTimer < diff)
							{
								...
								chaseTimer = 200;
								me->GetMotionMaster()->MovePoint(0, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ() + 3.0f);
							}
							else 
								chaseTimer -= diff;
						}
	...
	public:
		...
		struct npc_kinetic_bombAI : public ScriptedAI
		{
			...
			void Reset()
			{
				...
				_groundZ = me->GetMap()->GetHeight(_x, _y, _groundZ, true, 500.0f) - 2.0f;
			}
			...
			void DoAction(int32 const action)
			{
				...
				else if (action == ACTION_KINETIC_BOMB_JUMP)
				{
					...
					float jump = std::min(me->GetPositionZ() + 7.0f, 385.0f);
					continueFall = uint32(std::max(0.0f, 100*(jump-me->GetPositionZ())));
					me->GetMotionMaster()->MoveJump(_x, _y, jump, 10.0f, 1.0f);
				}
			}
			...
			void UpdateAI(uint32 const diff)
			{
				...
				if (!exploded && me->GetPositionZ() - 2.0f < _groundZ)
				{
					exploded = true;
