			...
			void MovementInform(uint32 type, uint32 pointId)
			{
				...
				switch (pointId)
				{
					...
					case POINT_CENTER_1:
						...
						events.ScheduleEvent(EVENT_SUMMON_VALKYR, 81500, 0, PHASE_TWO);
						events.ScheduleEvent(EVENT_INFEST, 70000, 0, PHASE_TWO);
						events.ScheduleEvent(EVENT_DEFILE, 101000, 0, PHASE_TWO);
					...
					switch (eventId)
					{
						...
						case EVENT_NECROTIC_PLAGUE:
							if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, NecroticPlagueTargetCheck(me, NECROTIC_PLAGUE_LK, NECROTIC_PLAGUE_PLR)))
							{
								...
								sLog->outError("No valid target for Necrotic Plague", 0);
							events.ScheduleEvent(EVENT_NECROTIC_PLAGUE, urand(30000, 33000), 0, PHASE_ONE);
							break;
						...
						case EVENT_SUMMON_VALKYR:
							...
							events.ScheduleEvent(EVENT_SUMMON_VALKYR, 48000, 0, PHASE_TWO);
							break;
...
class npc_valkyr_shadowguard : public CreatureScript
{
	public:
		...
		struct npc_valkyr_shadowguardAI : public ScriptedAI
		{
					...
					case POINT_CHARGE:
						if (Player* target = ObjectAccessor::GetPlayer(*me, _grabbedPlayer))
						{
							...
							if (GameObject* platform = ObjectAccessor::GetGameObject(*me, _instance->GetData64(DATA_ARTHAS_PLATFORM)))
							{
								...
								target->RemoveAurasDueToSpell(SPELL_INFEST);
								DoCast(target, SPELL_VALKYR_CARRY);
								_dropPoint.Relocate(triggers.front());
								_events.ScheduleEvent(EVENT_MOVE_TO_DROP_POS, 50);
			...
			{
				_grabbedPlayer = guid;
				if (Unit* target = Unit::GetUnit(*me, _grabbedPlayer))
					me->GetMotionMaster()->MoveCharge(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 25.0f, POINT_CHARGE);
			}
				...
				while (uint32 eventId = _events.ExecuteEvent())
				{
					switch (eventId)
					{
						...
						case EVENT_MOVE_TO_DROP_POS:
							...
							//_events.ScheduleEvent(EVENT_MOVE_TO_DROP_POS, 250);
							break;
		...
		class spell_the_lich_king_necrotic_plague_AuraScript : public AuraScript
		{
			...
			void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
			{
				...
				if (InstanceScript* instance = GetCaster()->GetInstanceScript())
					if (Unit* lichking = Unit::GetUnit(*GetCaster(), instance->GetData64(DATA_THE_LICH_KING)))
...
class spell_the_lich_king_valkyr_target_search : public SpellScriptLoader
{
	public:
		...
		class spell_the_lich_king_valkyr_target_search_SpellScript : public SpellScript
		{
			...
			void ReplaceTarget(std::list<Unit*>& targets)
			{
				targets.clear();
				if (_target)
					targets.push_back(_target);
