							...
							if (!summons.empty())
							{
								if (!apply)
								{
									...
									for (std::list<uint64>::iterator ite = summons.begin(); ite != summons.end(); ite++)
										if (Unit* summon = Unit::GetUnit(*me, *ite))
										{
											summon->RemoveAurasDueToSpell(72771);
											summon->AddAura(25176, summon);
										}
									apply = true;
									events.ScheduleEvent(EVENT_SCENT_OF_BLOOD, 8000, 0, PHASE_COMBAT);
								}
								else
									for (std::list<uint64>::iterator ite = summons.begin(); ite != summons.end(); ite++)
										if (Unit* summon = Unit::GetUnit(*me, *ite))
											summon->RemoveAurasDueToSpell(25176);
							}
							break;
						default:
							break;
