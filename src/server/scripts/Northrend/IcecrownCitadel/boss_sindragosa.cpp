...
Position const SindragosaDespawnPos  = {4525.600f, 2485.150f, 245.0820f, 0.0f};
...
class boss_sindragosa : public CreatureScript
{
	public:
		...
		struct boss_sindragosaAI : public BossAI
		{
			...
			void JustDied(Unit* killer)
			{
				...
				for (Map::PlayerList::const_iterator itr = map->GetPlayers().begin(); itr != map->GetPlayers().end(); ++itr)
				{
					if (Player* player = itr->getSource())
						if (player->GetQuestStatus(QUEST_FROST_INFUSION) == QUEST_STATUS_INCOMPLETE && player->HasAura(SPELL_FROST_IMBUED_BLADE))
			...
			{
				if (Unit* rimefang = Unit::GetUnit(*me, instance->GetData64(DATA_RIMEFANG)))
					if (!rimefang->isAlive())
			...
			void JustReachedHome()
			{
				...
				me->GetMotionMaster()->MoveTakeoff(POINT_TAKEOFF_DESPAWN, pos, 8.30078125f);
			}
			...
			void MovementInform(uint32 type, uint32 point)
			{
				...
				switch (point)
				{
					...
					case POINT_TAKEOFF_DESPAWN:
					{
						moveDespawn = true;
						
						std::list<Creature*> whelpList;
						me->GetCreatureListWithEntryInGrid(whelpList, NPC_FROSTWING_WHELP, 200.0f);
						...
						{
							rimefang->SetVisible(true);
							rimefang->Respawn();
							rimefang->SetCanFly(true);
							rimefang->SetDisableGravity(true);
							rimefang->SetByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER);
						}
						
						if (Creature* spinestalker = Unit::GetCreature(*me, instance->GetData64(DATA_SPINESTALKER)))
						{
							spinestalker->SetVisible(true);
							spinestalker->Respawn();
							spinestalker->SetCanFly(true);
							spinestalker->SetDisableGravity(true);
							spinestalker->SetByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER);
						}
						break;
					}
					case POINT_DESPAWN:
								...
								if (!player->HasAura(SPELL_FROST_IMBUED_BLADE) && shadowsEdge->GetEntry() == ITEM_SHADOW_S_EDGE)
								{
									if (Aura* infusion = player->GetAura(SPELL_FROST_INFUSION))
									{
										if (infusion->GetStackAmount() == 3)
										{
											...
											me->AddAura(SPELL_FROST_INFUSION, player);
									}
									else
										me->AddAura(SPELL_FROST_INFUSION, player);
			...
			void UpdateAI(uint32 const diff)
			{
				if (!UpdateVictim() || !CheckInRoom())
				{
					if (moveDespawn)
					{
						me->GetMotionMaster()->MovePoint(POINT_DESPAWN, SindragosaDespawnPos);
						moveDespawn = false;
					}
					return;
				}
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch (eventId)
					{
						...
						case EVENT_AIR_MOVEMENT_FAR:
							...
							break;
						case EVENT_ICE_TOMB:
							if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, TargetSelectorTankPlayerDistancesSpell(me, true, true, 0.0f, 0.0f, -SPELL_ICE_TOMB_UNTARGETABLE)))
							{
						...
						case EVENT_LAND_GROUND:
							...
							me->GetMotionMaster()->MoveLand(POINT_LAND_GROUND, SindragosaLandPos, 8.30078125f);
							break;
...
#define GOSSIP_ITEM_1  "Call Sindragosa"
class npc_sindragosa_starter : public CreatureScript
{
	public:
		npc_sindragosa_starter() : CreatureScript("npc_sindragosa_starter") { }

		struct npc_sindragosa_starterAI : public ScriptedAI
		{
			npc_sindragosa_starterAI(Creature* creature) : ScriptedAI(creature)
			{
		...
		bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 action)
		{
			...
			if (action==GOSSIP_ACTION_INFO_DEF+1)
				...
				{
					if (Creature* boss = creature->SummonCreature(NPC_SINDRAGOSA, SindragosaSpawnPos))
		...
		CreatureAI* GetAI(Creature* creature) const
		{
			return new npc_sindragosa_starterAI(creature);
		}
};
...
class npc_ice_tomb : public CreatureScript
{
	public:
		...
		struct npc_ice_tombAI : public Scripted_NoMovementAI
		{
			...
			void SetGUID(uint64 guid, int32 type/* = 0 */)
			{
				if (type == DATA_TRAPPED_PLAYER)
				{
					...
					if (Unit* summoner = me->ToTempSummon()->GetSummoner())
						...
						{
							_trappedPlayerThreat = summoner->getThreatManager().getThreat(player);
							summoner->getThreatManager().modifyThreatPercent(player, -100);
						}
				}
			}
			...
			void DoAction(int32 const action)
			{
				if (action == ACTION_TRIGGER_ASPHYXIATION)
					if (Player* player = ObjectAccessor::GetPlayer(*me, _trappedPlayerGUID))
						me->AddAura(SPELL_ASPHYXIATION, player);
			}
			
			void DamageDealt(Unit* target, uint32& damage, DamageEffectType /*damageType*/)
			{
				if (damage >= target->GetHealth())
					if (target->HasAura(SPELL_ASPHYXIATION))
			...
			void JustDied(Unit* /*killer*/)
			{
				...
				if (Player* player = ObjectAccessor::GetPlayer(*me, _trappedPlayerGUID))
				{
					...
					if (player->isAlive() && summoner->isInCombat())
						summoner->AddThreat(player, _trappedPlayerThreat);
				}
			}
			...
			void UpdateAI(uint32 const diff)
			{
				...
				if (_asphyxationTimer)
				{
					if (_asphyxationTimer <= diff)
					{
						DoAction(ACTION_TRIGGER_ASPHYXIATION);
						_asphyxationTimer = 0;
					} else _asphyxationTimer -= diff;
				}
			}
		...
		private:
			...
			uint32 _asphyxationTimer;
			float _trappedPlayerThreat;
		};
				...
				while (uint32 eventId = _events.ExecuteEvent())
				{
					switch (eventId)
					{
						...
						case EVENT_ICY_BLAST:
						{
							...
							me->GetMotionMaster()->MovePoint(POINT_FROSTWYRM_FLY_IN, RimefangFlyPos);
							float moveTime = me->GetExactDist(&RimefangFlyPos)/(me->GetSpeed(MOVE_FLIGHT)*0.001f);
							_events.DelayEvents(urand(60000, 70000));
	...
	public:
		...
		class spell_sindragosa_s_fury_SpellScript : public SpellScript
		{
			...
			void HandleDummy(SpellEffIndex effIndex)
			{
				...
				uint32 damage = (uint32(GetEffectValue()  * (GetCaster()->GetMap()->Is25ManRaid() ? 2.5f : 1.0f) / _targetCount) * randomResist) / 10;
...
class TombTargetDeselector
{
	public:
		TombTargetDeselector(Unit* caster, std::list<Creature*> const& collisionList) : _caster(caster), _collisionList(collisionList) { }
	...
	public:
		...
		class spell_sindragosa_collision_filter_SpellScript : public SpellScript
		{
			...
			void FilterTargets(std::list<Unit*>& unitList)
			{
				...
				tombs.remove_if (DeadTombDeselector());
				unitList.remove_if (TombTargetDeselector(GetCaster(), tombs));
			}
	...
	public:
		...
		class spell_sindragosa_mystic_buffet_SpellScript : public SpellScript
		{
			...
			void FilterTargets(std::list<Unit*>& targets)
			{
				...
				GetCreatureListWithEntryInGrid(tombs, GetCaster(), NPC_ICE_TOMB, 200.0f);
				tombs.remove_if (DeadTombDeselector());
				targets.remove_if (TombTargetDeselector(GetCaster(), tombs));
...
class at_sindragosa_lair : public AreaTriggerScript
{
	public:
		...
		bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/)
		{
			...
			{
				instance->SetData(DATA_SINDRAGOSA_PORT_ELIGIBLE, DONE);
