...
class boss_professor_putricide : public CreatureScript
{
	public:
		...
		struct boss_professor_putricideAI : public BossAI
		{
			...
			{
				instance->DoRemoveAurasDueToSpellOnPlayers(sSpellMgr->GetSpellIdForDifficulty(SPELL_MUTATED_PLAGUE, me));
				_JustDied();
				Talk(SAY_DEATH);
				summons.DespawnAll();
			...
			void JustSummoned(Creature* summon)
			{
				...
				switch (summon->GetEntry())
				{
					...
					case NPC_GAS_CLOUD:
						...
						instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_GASEOUS_BLOAT_PROTECTION);
						...
						summon->SetReactState(REACT_DEFENSIVE);//REACT_DEFENSIVE, REACT_PASSIVE
						return;
					case NPC_VOLATILE_OOZE:
						// no possible aura seen in sniff adding the aurastate
						instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_VOLATILE_OOZE_PROTECTION);
					...
					case NPC_MUTATED_ABOMINATION_25:
						abominationGUID = summon->GetGUID();
						return;
					default:
						break;
				}
				...
				switch (action)
				{
					...
					case ACTION_ROTFACE_COMBAT:
					{
						...
						if (Creature* rotface = Unit::GetCreature(*me, instance->GetData64(DATA_ROTFACE)))
						{
							...
							GetCreatureListWithEntryInGrid(list, rotface, NPC_PUDDLE_MIDDLE, 50.0f);
							uint8 i = 0;
							for (std::list<Creature*>::iterator ite = list.begin(); ite != list.end(); ite++)
							{
								_oozeFloodDummyGUIDs[i++] = (*ite)->GetGUID();
							}
						}
						break;
					}
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch (eventId)
					{
						...
						case EVENT_RESUME_ATTACK:
							...
							if (abominationGUID)
								if (Unit* abomination = Unit::GetUnit(*me, abominationGUID))
								{
									abomination->RemoveAurasDueToSpell(sSpellMgr->GetSpellIdForDifficulty(SPELL_TEAR_GAS_CREATURE, me));
									abomination->RemoveAurasDueToSpell(sSpellMgr->GetSpellIdForDifficulty(71615, me));
								}
	...
	public:
		...
		struct npc_putricide_oozeAI : public ScriptedAI
		{
			...
			void UpdateAI(uint32 const diff)
			{
				...
				if (_channelTarget)
					if (Unit* target = Unit::GetUnit(*me, _channelTarget))
						if (!target->isAlive())
						{
							_channelTarget = 0;
							_newTargetSelectTimer = 1000;
...
class npc_gas_cloud : public CreatureScript
{
	public:
		...
		struct npc_gas_cloudAI : public ScriptedAI
		{
			...
			void DoAction(int32 const action)
			{
				if (action == ACTION_STACK_EXPIRED)
				{
					_newTargetSelectTimer = 1000;
					_channelTarget = 0;
				}
			...
			void HandleExtraEffect(AuraEffect const* /*aurEff*/)
			{
				...
				if (Unit* caster = GetCaster())
				{
					...
					if (!target->HasAura(GetId()))
						caster->GetAI()->DoAction(ACTION_STACK_EXPIRED);
	...
	public:
		...
		class spell_putricide_ooze_channel_SpellScript : public SpellScript
		{
			...
			void StartAttack()
			{
				...
				GetCaster()->GetAI()->SetGUID(GetHitUnit()->GetGUID(), CHANNEL_TARGET);
			}
	...
	public:
		...
		class spell_putricide_unstable_experiment_SpellScript : public SpellScript
		{
			...
			void HandleScript(SpellEffIndex effIndex)
			{
				...
				uint32 stage = GetCaster()->GetAI()->GetData(DATA_EXPERIMENT_STAGE);
		//0=ooze
		//1=gas
	...
	public:
		...
		class spell_putricide_mutated_plague_AuraScript : public AuraScript
		{
			...
			void HandleTriggerSpell(AuraEffect const* aurEff)
			{
				...
				damage *= int32(pow(2.4f, GetStackAmount()-2));
			...
			void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
			{
				...
				uint32 healSpell;
				switch (Difficulty(GetCaster()->GetMap()->GetSpawnMode()))
				{
				...
				if (InstanceScript* instance = GetCaster()->GetInstanceScript())
					if (Unit* profesor = Unit::GetUnit(*GetCaster(), instance->GetData64(DATA_PROFESSOR_PUTRICIDE)))
						profesor->DealHeal(profesor, healSpell*GetStackAmount());
						//profesor->CastSpell(profesor, healSpell*GetStackAmount(), true, NULL, NULL, GetCasterGUID());
			}
