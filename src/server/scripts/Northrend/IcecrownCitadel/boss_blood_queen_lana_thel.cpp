...
enum Spells
{
	...
	SPELL_SWARMING_SHADOWS_EFFECT           = 71267,
		...
		struct boss_blood_queen_lana_thelAI : public BossAI
		{
			...
			void JustDied(Unit* killer)
			{
				...
				for (Map::PlayerList::const_iterator itr = map->GetPlayers().begin(); itr != map->GetPlayers().end(); ++itr)
				{
					if (Player* player = itr->getSource())
						if (player->GetQuestStatus(QUEST_BLOOD_INFUSION) == QUEST_STATUS_INCOMPLETE && player->HasAura(SPELL_THIRST_QUENCHED))
							player->CompleteQuest(QUEST_BLOOD_INFUSION);
				}
			}
			...
			void KilledUnit(Unit* victim)
			{
				if (victim->GetTypeId() == TYPEID_PLAYER)
				{
					Talk(SAY_KILL);
					if (IsHeroic())
						if (WasVampire(victim->GetGUID()))
							if (Aura* aura = me->GetAura(SPELL_PRESENCE_OF_THE_DARKFALLEN))
							{
								aura->ModStackAmount(-1);
								for (std::set<uint64>::iterator ite = _vampires.begin(); ite != _vampires.end(); ite++)
								{
									if (*ite == victim->GetGUID())
										_vampires.erase(ite);
								}
							}
				}
			...
			void SetGUID(uint64 guid, int32 type = 0)
			{
				...
				{
					case GUID_VAMPIRE:
						if (_vampires.count(guid) == 0)
							_vampires.insert(guid);
						break;
					case GUID_BLOODBOLT:
						if (_bloodboltedPlayers.count(guid) == 0)
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch (eventId)
					{
								...
								if (_offtank != newOfftank)
								{
									...
									if (_offtank)
									{
										...
										if (Item* shadowsEdge = _offtank->GetWeaponForAttack(BASE_ATTACK, true))
											if (Is25ManRaid() && !_offtank->HasAura(SPELL_THIRST_QUENCHED) && shadowsEdge->GetEntry() == ITEM_SHADOW_S_EDGE && !_offtank->HasAura(SPELL_GUSHING_WOUND))
						...
						case EVENT_TWILIGHT_BLOODBOLT:
						{
							...
							for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
								DoCast(*itr, sSpellMgr->GetSpellIdForDifficulty(SPELL_TWILIGHT_BLOODBOLT, me));
...
class npc_swarming_shadow : public CreatureScript
{
public:
	npc_swarming_shadow() : CreatureScript("npc_swarming_shadow") { }
	
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_swarming_shadowAI(creature);
	}
	
	struct npc_swarming_shadowAI : public Scripted_NoMovementAI
	{
		npc_swarming_shadowAI(Creature* creature) : Scripted_NoMovementAI(creature)
		{
			Reset();
		}
		...
		void Reset()
		{
			me->SetFloatValue(OBJECT_FIELD_SCALE_X, 1.4f);
			me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
			me->SetInCombatWithZone();
			DoCast(me, SPELL_SWARMING_SHADOWS_EFFECT);
		}
				...
				if (GetCaster()->GetMap()->IsHeroic())
					if (InstanceScript* instance = GetCaster()->GetInstanceScript())
						if (Creature* bloodQueen = ObjectAccessor::GetCreature(*GetCaster(), instance->GetData64(DATA_BLOOD_QUEEN_LANA_THEL)))
							bloodQueen->AddAura(SPELL_PRESENCE_OF_THE_DARKFALLEN, bloodQueen);
	...
	public:
		...
		class spell_blood_queen_pact_of_the_darkfallen_SpellScript : public SpellScript
		{
			...
			void FilterTargets(std::list<Unit*>& targets)
			{
				...
				bool remove = true;
				std::list<Unit*>::const_iterator itrEnd = targets.end(), itr, itr2;
