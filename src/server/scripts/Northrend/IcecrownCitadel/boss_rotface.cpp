...
class boss_rotface : public CreatureScript
{
	public:
		...
		struct boss_rotfaceAI : public BossAI
		{
			...
			{
				if (spell->Id == sSpellMgr->GetSpellIdForDifficulty(69507, me))//Slime spray effect
					if (target->GetTypeId() == TYPEID_PLAYER)
						target->AddAura(SPELL_GREEN_BLIGHT_RESIDUE, target);
			}
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch (eventId)
					{
						case EVENT_SLIME_SPRAY:
							...
							{
								stalkerGUID = me->SummonCreature(NPC_OOZE_SPRAY_STALKER, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 0.0f, TEMPSUMMON_MANUAL_DESPAWN, 0)->GetGUID();
							...
							events.ScheduleEvent(EVENT_SLIME_SPRAY_END, 6500);
							events.ScheduleEvent(EVENT_SLIME_SPRAY, 20000);
							break;
						case EVENT_SLIME_SPRAY_END:
							if (Creature* stalker = Unit::GetCreature(*me, stalkerGUID))
						...
						case EVENT_ROTFACE_VILE_GAS:
						{
							...
							if (Creature* professor = Unit::GetCreature(*me, instance->GetData64(DATA_PROFESSOR_PUTRICIDE)))
								professor->CastSpell(target, SPELL_VILE_GAS_H, true);
							events.ScheduleEvent(EVENT_ROTFACE_VILE_GAS, urand(28000, 35000));
							break;
						}
						case EVENT_HASTEN_INFECTIONS:
							if (infectionCooldown>3000)
							{
						...
						case EVENT_MUTATED_INFECTION:
							for (uint8 i=0; i<RAID_MODE<uint32>(1, 2, 1, 2); i++) {
								if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM)) {
									target->AddAura(SPELL_MUTATED_INFECTION, target);
								}
							}
	...
	public:
		...
		struct npc_precious_iccAI : public ScriptedAI
		{
			...
			void Reset()
			{
				...
				_events.ScheduleEvent(EVENT_SUMMON_ZOMBIES, urand(20000, 25000));
				_summons.DespawnAll();
			}
				...
				while (uint32 eventId = _events.ExecuteEvent())
				{
					switch (eventId)
					{
						...
						case EVENT_SUMMON_ZOMBIES:
							...
							_events.ScheduleEvent(EVENT_DECIMATE, 1000);
							_events.ScheduleEvent(EVENT_SUMMON_ZOMBIES, urand(20000, 25000));
							break;
						default:
							break;
	...
	public:
		...
		class spell_rotface_ooze_flood_SpellScript : public SpellScript
		{
			...
			void HandleScript(SpellEffIndex /*effIndex*/)
			{
				...
				GetHitUnit()->CastSpell(GetHitUnit(), uint32(GetEffectValue()), false, NULL, NULL, GetOriginalCaster() ? GetOriginalCaster()->GetGUID() : 0);
			}
			...
			void HandleScript(SpellEffIndex /*effIndex*/)
			{
				...
				if (Aura* unstable = GetCaster()->GetAura(SPELL_UNSTABLE_OOZE))
					if (Aura* targetAura = GetHitCreature()->GetAura(SPELL_UNSTABLE_OOZE))
						if (unstable->GetStackAmount() <= targetAura->GetStackAmount()) {
	...
	public:
		...
		class spell_rotface_unstable_ooze_explosion_init_SpellScript : public SpellScript
		{
			...
			void HandleCast(SpellEffIndex effIndex)
			{
				...
				Creature* dummy = GetCaster()->SummonCreature(NPC_UNSTABLE_EXPLOSION_STALKER, x, y, z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 15000);
