	...
	public:
		...
		struct boss_lady_deathwhisperAI : public BossAI
		{
					...
					switch (eventId)
					{
						...
						case EVENT_DOMINATE_MIND_H:
							...
							for (uint8 i = 0; i < _dominateMindCount; i++)
								if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, TargetSelectorTankPlayerDistancesSpell(me, true, true, 0.0f, 0.0f, -SPELL_DOMINATE_MIND_H)))
			...
			void SetGUID(uint64 guid, int32 id)
			{
				...
				events.ScheduleEvent(EVENT_P1_REANIMATE_CULTIST, 2000);
			}
			...
			void ReanimateCultist()
			{
				...
				if (cultist->GetEntry() == NPC_CULT_FANATIC)
					me->SummonCreature(NPC_REANIMATED_FANATIC, cultist->GetPositionX(), cultist->GetPositionY(), cultist->GetPositionZ());
				else
					me->SummonCreature(NPC_REANIMATED_ADHERENT, cultist->GetPositionX(), cultist->GetPositionY(), cultist->GetPositionZ());
	...
	public:
		...
		struct npc_cult_fanaticAI : public ScriptedAI
		{
			...
			void UpdateAI(uint32 const diff)
			{
				...
				if (me->GetDisplayId() == 30968 && me->GetEntry() == NPC_CULT_FANATIC) {
					if (me->isSummon())
	...
	public:
		...
		struct npc_cult_adherentAI : public ScriptedAI
		{
			...
			void UpdateAI(uint32 const diff)
			{
				...
				if (me->GetDisplayId() == 30966 && me->GetEntry() == NPC_CULT_ADHERENT) {
					if (me->isSummon())
						if (Unit* owner = me->ToTempSummon()->GetSummoner())
							owner->GetAI()->SetGUID(me->GetGUID(), GUID_CULTIST);
					me->Kill(me);
