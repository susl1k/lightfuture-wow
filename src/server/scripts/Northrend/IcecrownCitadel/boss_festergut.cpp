	...
	public:
		...
		struct boss_festergutAI : public BossAI
		{
			...
			void Reset()
			{
				...
				events.ScheduleEvent(EVENT_GASTRIC_BLOAT, 12000);
				events.ScheduleEvent(EVENT_VILE_GAS, urand(28000, 35000), EVENT_VILE_GROUP);
				_maxInoculatedStack = 0;
				_inhaleCounter = 0;
			...
			void SpellHitTarget(Unit* target, SpellInfo const* spell)
			{
				...
				if (spell->Id == sSpellMgr->GetSpellIdForDifficulty(69159, me))//Gaseous Blight effect
					if (target->GetTypeId() == TYPEID_PLAYER)
						me->AddAura(SPELL_ORANGE_BLIGHT_RESIDUE, target);
			}
					...
					switch (eventId)
					{
						case EVENT_INHALE_BLIGHT:
						{
							...
							if (_inhaleCounter == 3)
							{
								...
								events.RescheduleEvent(EVENT_GAS_SPORE, 20000);
							}
							else
							{
						...
						case EVENT_VILE_GAS:
						{
							Unit* target=SelectRandomVileGasTarget();
							if (!target)
						...
						case EVENT_GAS_SPORE:
						{
							...
							std::list<Unit*> list;
							SelectTargetList(list, RAID_MODE<int32>(2, 3, 2, 3), SELECT_TARGET_RANDOM, 0.0f, true);
							if (!list.empty())
								for (std::list<Unit*>::iterator ite = list.begin(); ite != list.end(); ite++)
									me->AddAura(69279, *ite);
							//me->CastCustomSpell(SPELL_GAS_SPORE, SPELLVALUE_MAX_TARGETS, RAID_MODE<int32>(2, 3, 2, 3), me);
							events.ScheduleEvent(EVENT_GAS_SPORE, 40000);
							events.DelayEvents(15000, EVENT_VILE_GROUP);
							break;
						}
			...
			Player* SelectRandomVileGasTarget() {
				...
				for (std::list<HostileReference*>::const_iterator itr = threatlist.begin(); itr != threatlist.end(); ++itr)
					if (Unit* refTarget = (*itr)->getTarget())
						if (me->GetExactDist2d(refTarget->GetPositionX(), refTarget->GetPositionY()) > 8.0f)
							targets.push_back(refTarget->ToPlayer());
				
				if (targets.size() < RAID_MODE<uint32>(3, 8, 3, 8))
					return NULL;
...
class spell_festergut_gas_spore : public SpellScriptLoader
{
	public:
		spell_festergut_gas_spore() : SpellScriptLoader("spell_festergut_gas_spore") { }

		class spell_festergut_gas_spore_AuraScript : public AuraScript
		{
			PrepareAuraScript(spell_festergut_gas_spore_AuraScript);
			...
			void HandleTriggerSpell(AuraEffect const* aurEff)
			{
				...
				if (InstanceScript* instance = GetTarget()->GetInstanceScript())
					if (Creature* festergut = ObjectAccessor::GetCreature(*GetTarget(), instance->GetData64(DATA_FESTERGUT)))
						GetTarget()->CastSpell(GetTarget(), sSpellMgr->GetSpellIdForDifficulty(SPELL_BLIGHTED_SPORES, festergut), true, NULL, NULL, festergut->GetGUID());
			}

			void Register()
			{
				OnEffectPeriodic += AuraEffectPeriodicFn(spell_festergut_gas_spore_AuraScript::HandleTriggerSpell, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
			}
		};

		AuraScript* GetAuraScript() const
		{
			return new spell_festergut_gas_spore_AuraScript();
		}
};
	...
	public:
		...
		class spell_festergut_blighted_spores_AuraScript : public AuraScript
		{
			...
			void ExtraEffect(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
			{
				...
				if (!GetTarget()->HasAura(inoculatedID))
					GetTarget()->CastSpell(GetTarget(), inoculatedID, true);
				else
				{
					Aura* inoculated = GetTarget()->GetAura(inoculatedID);
					if (inoculated->GetMaxDuration() > inoculated->GetDuration() + 5000)
					{
						inoculated->ModStackAmount(1);
						if (InstanceScript* instance = GetTarget()->GetInstanceScript())
							if (Creature* festergut = ObjectAccessor::GetCreature(*GetTarget(), instance->GetData64(DATA_FESTERGUT)))
								if (Aura const* aura = GetTarget()->GetAura(inoculatedID))
									festergut->AI()->SetData(DATA_INOCULATED_STACK, aura->GetStackAmount());
					}
				}
			}
