...
enum Actions
{
	...
	ACTION_MAGE_DIE,
	ACTION_ROCK_DIE,
	ACTION_AXES_RIFL_DIE,
	ACTION_DONE,
	ACTION_FAIL,
	ACTION_HORDE_VERSION_START,
	ACTION_HORDE_VERSION_DONE,
	ACTION_ALLIANCE_VERSION_START,
	ACTION_ALLIANCE_VERSION_DONE,
	ACTION_RESET,
	ACTION_FROSTWYRM_LAND,
};
...
Position const FrostWyrmLandH  = {-437.445f, 2078.454f, 191.2549f, 2.286815f};
Position const FrostWyrmLandA  = {-437.317f, 2344.641f, 191.2549f, 3.529749f};
...
void StopFight(Transport* t1, Transport* t2)
{
	...
	for (Transport::CreatureSet::iterator itr = t1->m_NPCPassengerSet.begin(); itr != t1->m_NPCPassengerSet.end(); ++itr)
	{
		if (Creature *npc = *itr)
		{
			if (npc->GetEntry() == NPC_GB_SKYBREAKER || npc->GetEntry() == NPC_GB_ORGRIMS_HAMMER)
				npc->CombatStop();
	...
	for (Transport::CreatureSet::iterator itr = t2->m_NPCPassengerSet.begin(); itr != t2->m_NPCPassengerSet.end(); ++itr)
	{
		if (Creature *npc = *itr)
		{
...
class npc_muradin_gunship : public CreatureScript
{
	public:
		...
		bool OnGossipHello(Player* player, Creature* pCreature)
		{
			...
			{
				if (!pCreature->AI()->GetData(DATA_EVENT_DONE))
				{
					if ((!player->GetGroup() || !player->GetGroup()->IsLeader(player->GetGUID())) && !player->isGameMaster())
					{
						player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "You are not the leader of the party...", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
				...
				else
				{
					...
					player->ADD_GOSSIP_ITEM(0, "Teleport us to the entrance.", 631, 1011);
					player->SEND_GOSSIP_MENU(player->GetGossipTextId(pCreature), pCreature->GetGUID());
				}
				return true;
			}
		...
		bool OnGossipSelect(Player* player, Creature* pCreature, uint32 /*sender*/, uint32 action)
		{
			...
			if (action == GOSSIP_ACTION_INFO_DEF+2)
				pCreature->MonsterSay("Only party leader can do that.", LANG_UNIVERSAL, player->GetGUID());
			...
			else if (action == 1011)
			{
				...
				for (Map::PlayerList::const_iterator itr = map->GetPlayers().begin(); itr != map->GetPlayers().end(); ++itr)
					if (Player* pPlayer = itr->getSource())
					{
						pPlayer->SetTransport(NULL);
						pPlayer->CombatStop();
						pPlayer->CastSpell(pPlayer, 5000, true);
					}
			}
			return true;
		}
		...
		struct npc_muradin_gunshipAI : public ScriptedAI
		{
			...
			{
				if (type != DATA_EVENT_DONE)
					return 0;
				
				return EventDone ? 1 : 0;
			}
			...
			void SetGUID(uint64 guid, int32 type)
			{
				switch (type)
				{
					...
					case ACTION_AXES_RIFL_DIE:
						RespawnQueue.push_back(guid);
						events.ScheduleEvent(EVENT_RESPAWN_PASSAGER, 35000);
					break;
				}
			}
			...
			void DoAction(int32 const action)
			{
				switch (action)
				{
					...
					case ACTION_HORDE_VERSION_START:
					{
						me->SummonCreature(2000120, -422.0f, 2476.0f, 470.5f, M_PI);//utocici velitel druhy lode
						me->SetVisible(false);
						me->setFaction(35);
						
						me->SummonCreature(NPC_GB_SKYBREAKER_RIFLEMAN, -461.0f, 2450.0f, 471.05f, 4.857f);
						me->SummonCreature(NPC_GB_SKYBREAKER_RIFLEMAN, -455.0f, 2450.0f, 471.05f, 4.857f);
						me->SummonCreature(NPC_GB_SKYBREAKER_RIFLEMAN, -448.0f, 2451.0f, 471.05f, 4.857f);
						me->SummonCreature(NPC_GB_SKYBREAKER_RIFLEMAN, -438.0f, 2452.0f, 471.05f, 4.857f);
						if (Is25ManRaid())
						{
							me->SummonCreature(NPC_GB_SKYBREAKER_RIFLEMAN, -466.0f, 2449.0f, 471.05f, 4.857f);
							me->SummonCreature(NPC_GB_SKYBREAKER_RIFLEMAN, -433.0f, 2452.0f, 471.05f, 4.857f);
						}
						
						events.RescheduleEvent(EVENT_SPAWN_MAGE, 20000);
						break;
					}
					case ACTION_HORDE_VERSION_DONE:
						me->SetVisible(true);
						me->setFaction(21);
						break;
					case ACTION_DONE:
						if (Creature* saurfang = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_GB_HIGH_OVERLORD_SAURFANG)))
							saurfang->AI()->DoAction(ACTION_ALLIANCE_VERSION_DONE);
					...
					case ACTION_MAGE_DIE:
						events.RescheduleEvent(EVENT_SPAWN_MAGE, 35000);
						break;
				}
			}
			...
			{
				if (_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE && damage > me->GetHealth())
				   me->AI()->DoAction(ACTION_FAIL);
					...
					switch (eventId)
					{
						case EVENT_WIPE_CHECK:
							...
							if (DoWipeCheck(skybreaker))
								events.ScheduleEvent(EVENT_WIPE_CHECK, 2000);
							else
								me->AI()->DoAction(ACTION_FAIL);
							break;
						...
						case EVENT_SUMMON_PORTAL:
							if (!EventDone)
							{
								...
								CheckUnfriendlyShip(me,_instance, DATA_GB_HIGH_OVERLORD_SAURFANG)->AddNPCPassengerInInstance(NPC_GB_PORTAL, 32.18615f, 0.00016f, 38.78849f, 3.13683f);
								skybreaker->AddNPCPassengerInInstance(NPC_GB_PORTAL, -15.51547f, -0.160213f, 28.87252f, 1.56211f)->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
							}
							break;
						...
						case EVENT_BOARDING_REAVERS_MARINE:
							if(!EventDone)
								if(count < SummonCount)
									...
									if(Creature* reavers = me->SummonCreature(NPC_GB_KORKRON_REAVERS, -391.0f, 2069.0f, 466.8f, 5.04f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000))
									{
						...
						case EVENT_OUTRO_ALLIANCE_3:
							me->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
							me->MonsterSay("I will teleport you to the entrance when you are ready.", LANG_UNIVERSAL, 0);
						...
						case EVENT_SPAWN_MAGE:
							...
							break;
						case EVENT_RESPAWN_PASSAGER:
						{
							uint64 respawnGUID = RespawnQueue.front();
							RespawnQueue.pop_front();
							if (Creature* Respawn = Unit::GetCreature(*me, respawnGUID))
							{
								Respawn->Respawn();
								Respawn->AI()->DoAction(ACTION_RESET);
								Respawn->SetInCombatWithZone();
							}
						}
						break;
					  }
...
class npc_saurfang_gunship : public CreatureScript
{
	public:
		...
		bool OnGossipHello(Player* player, Creature* pCreature)
		{
			...
			if (pInstance && pInstance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE)
			{
				if (!pCreature->AI()->GetData(DATA_EVENT_DONE))
				{
					if ((!player->GetGroup() || !player->GetGroup()->IsLeader(player->GetGUID())) && !player->isGameMaster())
					{
						player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "You are not leader of the party...", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
						player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, pCreature->GetGUID());
						return true;
					}
					
					player->ADD_GOSSIP_ITEM(0, "We are all ready Saurfang. Come on!", 631, 1001);
					player->SEND_GOSSIP_MENU(player->GetGossipTextId(pCreature), pCreature->GetGUID());
				}
				else
				{
		...
		bool OnGossipSelect(Player* player, Creature* pCreature, uint32 /*sender*/, uint32 action)
		{
			...
			if (action == GOSSIP_ACTION_INFO_DEF+2)
				pCreature->MonsterSay("Waiting for the leader of the party.", LANG_UNIVERSAL, player->GetGUID());
		...
		struct npc_saurfang_gunshipAI : public ScriptedAI
		{
			...
			void DoAction(int32 const action)
			{
				switch (action)
				{
					...
					case ACTION_ALLIANCE_VERSION_START:
					{
						me->SummonCreature(2000130, -349.0f, 2001.0f, 468.5f, M_PI);//utocici velitel druhy lode
						me->SetVisible(false);
						me->setFaction(35);
						
						me->SummonCreature(NPC_GB_KORKRON_AXETHROWER, -369.0f, 2021.0f, 467.6f, 1.86f);
						me->SummonCreature(NPC_GB_KORKRON_AXETHROWER, -384.0f, 2018.0f, 467.2f, 1.86f);
						me->SummonCreature(NPC_GB_KORKRON_AXETHROWER, -398.0f, 2013.0f, 465.9f, 1.86f);
						me->SummonCreature(NPC_GB_KORKRON_AXETHROWER, -404.0f, 2011.5f, 465.5f, 1.86f);
						if (Is25ManRaid())
						{
							me->SummonCreature(NPC_GB_KORKRON_AXETHROWER, -376.0f, 2020.0f, 467.2f, 1.86f);
							me->SummonCreature(NPC_GB_KORKRON_AXETHROWER, -414.5f, 2010.0f, 465.5f, 1.86f);
						}
						
						events.RescheduleEvent(EVENT_SPAWN_MAGE, 20000);
						break;
					}
					case ACTION_ALLIANCE_VERSION_DONE:
						me->SetVisible(true);
						me->setFaction(21);
						break;
					case ACTION_DONE:
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch (eventId)
					{
						...
						case EVENT_SUMMON_PORTAL:
							if (!EventDone)
							{
								CheckUnfriendlyShip(me,_instance, DATA_GB_MURADIN_BRONZEBEARD)->AddNPCPassengerInInstance(NPC_GB_PORTAL, 8.51547f, -0.160213f, 20.87252f, 3.10672f);
								orgrimmar->AddNPCPassengerInInstance(NPC_GB_PORTAL, 15.03016f, 0.00016f, 37.70952f, 1.55138f)->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);;
								events.ScheduleEvent(EVENT_BOARDING_TALK, 2500);
								events.ScheduleEvent(EVENT_BOARDING_GUNSHIP, 2500); // TODO: Fix the timers
							}
							break;
						case EVENT_BOARDING_TALK:
						...
						case EVENT_BOARDING_GUNSHIP:
							if (!EventDone)
							{
								count = 0;
								if (Creature* muradin = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_MURADIN_BRONZEBEARD_NOT_VISUAL)))
									muradin->AI()->Talk(SAY_BOARDING_SKYBREAKER_MURADIN);
								//if (Creature* sergante = orgrimmar->AddNPCPassengerInInstance(NPC_GB_SKYBREAKER_SERGANTE, 15.03016f, 0.00016f, 37.70952f, 1.55138f))
						...
						case EVENT_BOARDING_REAVERS_MARINE:
							if (!EventDone)
								if (count < SummonCount)
									//if (Creature* marine = orgrimmar->AddNPCPassengerInInstance(NPC_GB_SKYBREAKER_MARINE, 15.03016f, 0.00016f, 37.70952f, 1.55138f))
									if (Creature* marine = me->SummonCreature(NPC_GB_SKYBREAKER_MARINE, -423.0f, 2398.0f, 472.0f, 1.64f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000))
									{
...
class npc_korkron_axethrower_rifleman : public CreatureScript
{
	public:
		...
		struct npc_korkron_axethrower_riflemanAI : public Scripted_NoMovementAI
		{
			...
			void JustDied(Unit* killer)
			{
				if (Creature* boss = ObjectAccessor::GetCreature(*me, _instance->GetData64(_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE ? DATA_GB_HIGH_OVERLORD_SAURFANG : DATA_GB_MURADIN_BRONZEBEARD)))
					boss->AI()->SetGUID(me->GetGUID(), ACTION_AXES_RIFL_DIE);
			}
			...
			void UpdateAI(const uint32 diff)
			{
				...
				if (attacktimer <= diff)
				{
					...
					float timer = urand(6000, 15000);
					if (me->HasAura(SPELL_EXPERIENCED))
						timer /= 1.2f;
					if (me->HasAura(SPELL_ELITE))
						timer /= 1.4f;
					if (me->HasAura(SPELL_VETERAN))
						timer /= 1.8f;
					if (me->HasAura(DesperateResolve))
						timer /= 1.6f;
					attacktimer = uint32(timer);
...
class npc_sergeant : public CreatureScript
{
	public:
		...
		struct npc_sergeantAI : public ScriptedAI
		{
			...
			{
				if (target->GetTypeId() == TYPEID_PLAYER && target->GetPositionY() < 2437.0f && target->GetPositionY() > 2033.0f)
					return true;
				return false;
			}
	...
	public:
		...
		struct npc_gunship_mageAI : public Scripted_NoMovementAI
		{
			...
			void Reset()
			{
				...
				me->SetRespawnDelay(7*DAY);
				me->setFaction(21);
				timer_BelowZero = urand(3000, 4000);
				// Asignandole las armas por script para que no se caigan visualmente
			...
			void JustDied(Unit* killer)
			{
				if (me->GetGUID() == _instance->GetData64(DATA_GB_BATTLE_MAGE))
					if (Creature* boss = ObjectAccessor::GetCreature(*me, _instance->GetData64(_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE ? DATA_GB_HIGH_OVERLORD_SAURFANG : DATA_GB_MURADIN_BRONZEBEARD)))
						boss->AI()->DoAction(ACTION_MAGE_DIE);
			}
...
class npc_mortar_soldier_or_rocketeer : public CreatureScript
{
	public:
		...
		struct npc_mortar_soldier_or_rocketeerAI : public Scripted_NoMovementAI
		{
			...
			{
				if (Creature* boss = ObjectAccessor::GetCreature(*me, _instance->GetData64(_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE ? DATA_GB_HIGH_OVERLORD_SAURFANG : DATA_GB_MURADIN_BRONZEBEARD)))
					boss->AI()->SetGUID(me->GetGUID(), ACTION_ROCK_DIE);
			}
					...
					switch (eventId)
					{
						...
						case EVENT_ROCKET_ART:
						{
							if(Player* target = SelectRandomPlayerInTheMaps(me->GetMap()))
							{
								...
								else if(me->GetEntry() == NPC_GB_SKYBREAKER_MORTAR_SOLDIER)
								{
									me->CastSpell(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), SPELL_ROCKET_ARTILLERY_TRIGGERED, true);
									me->CastSpell(target, 70173, true);
								}
							}
							float timer = urand(10000, 15000);
							if (me->HasAura(SPELL_EXPERIENCED))
								timer /= 1.2f;
							if (me->HasAura(SPELL_ELITE))
								timer /= 1.4f;
							...
							if (me->HasAura(DesperateResolve))
								timer /= 1.6f;
							events.ScheduleEvent(EVENT_ROCKET_ART, uint32(timer));
						}
						break;
						default:
							break;
	...
	public:
		...
		bool OnGossipSelect(Player* player, Creature* pCreature, uint32 /*sender*/, uint32 action)
		{
			...
			if (action == 1)
			{
				...
				if (curItemCount >= 1)
				{
					pCreature->MonsterWhisper("You get the rocket pack!", player->GetGUIDLow());
					return false;
				}
				...
				{
					pCreature->MonsterWhisper("You do not have enough space to get rocket pack!", player->GetGUIDLow());
					return false;
				}
			}
...
class npc_gunship_portal : public CreatureScript
{
	public:
		...
		bool OnGossipHello(Player* player, Creature* pCreature) override
		{
			...
			{
				if (pCreature->GetInstanceScript()->GetBossState(DATA_GUNSHIP_EVENT) == IN_PROGRESS)
				{
					if (pCreature->GetInstanceScript()->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
					{
						if (Creature* pAllianceBoss = ObjectAccessor::GetCreature(*pCreature, pCreature->GetInstanceScript()->GetData64(DATA_SKYBREAKER_BOSS)))
						{
							sLog->outError("portuju hrace");
							player->NearTeleportTo(pAllianceBoss->GetPositionX(), pAllianceBoss->GetPositionY(), pAllianceBoss->GetPositionZ() + 5.0f, pAllianceBoss->GetOrientation());
						}
					}
					else
					{
						if (Creature* pHordeBoss = ObjectAccessor::GetCreature(*pCreature, pCreature->GetInstanceScript()->GetData64(DATA_ORGRIMMAR_HAMMER_BOSS)))
						{
							sLog->outError("portuju hrace");
							player->NearTeleportTo(pHordeBoss->GetPositionX(), pHordeBoss->GetPositionY(), pHordeBoss->GetPositionZ() + 5.0f, pHordeBoss->GetOrientation());
						}
					}
				}
			}
...
class npc_ramparts_npcs: public CreatureScript
{
	public:
		npc_ramparts_npcs() : CreatureScript("npc_ramparts_npcs") { }

		struct npc_ramparts_npcsAI : public ScriptedAI
		{
			npc_ramparts_npcsAI(Creature* creature) : ScriptedAI(creature)
			{
		...
		CreatureAI* GetAI(Creature* pCreature) const
		{
			return new npc_ramparts_npcsAI(pCreature);
		}
};
					...
					switch (eventId)
					{
						...
						case EVENT_HEAL:
							if (UpdateVictim())
							{
								...
								events.ScheduleEvent(EVENT_HEAL, 7000);
							}
							break;
					}
				}
...
class npc_icc_spire_frostwyrm: public CreatureScript
{
	public:
		...
		struct npc_icc_spire_frostwyrmAI : public ScriptedAI
		{
			...
			void DoAction(int32 const action)
			{
				if (action != ACTION_FROSTWYRM_LAND)
					return;
				
				if (instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE)
				{
					Talk(SAY_FROSTWYRM_LAND_H_0);
					me->GetMotionMaster()->MoveLand(0, FrostWyrmLandH, 10.0f);
				}
				else
				{
					Talk(SAY_FROSTWYRM_LAND_A_1);
					me->GetMotionMaster()->MoveLand(0, FrostWyrmLandA, 10.0f);
				}
			}
			...
			void MovementInform(uint32 type, uint32 point)
			{
				...
				me->SetHomePosition(instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE ? FrostWyrmLandH : FrostWyrmLandA);
		...
		bool OnTrigger(Player* player, AreaTriggerEntry const* areaTrigger)
		{
			if (InstanceScript* instance = player->GetInstanceScript())
				if (instance->GetData(DATA_SPIRE_FROSTWYRM_STATE) == NOT_STARTED && instance->GetData(DATA_SECOND_SQUAD_STATE) == DONE)
				{
					Creature* frostwyrm;
					if (instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
						frostwyrm = player->GetMap()->SummonCreature(NPC_SPIRE_FROSTWYRM, FrostWyrmPosA);
					else
						frostwyrm = player->GetMap()->SummonCreature(NPC_SPIRE_FROSTWYRM, FrostWyrmPosH);
					
					if (frostwyrm)
						frostwyrm->AI()->DoAction(ACTION_FROSTWYRM_LAND);
...
class npc_commander : public CreatureScript
{
	public:
		npc_commander() : CreatureScript("npc_commander") { }

		struct npc_commanderAI : public ScriptedAI
		{
			npc_commanderAI(Creature *creature) : ScriptedAI(creature)
			{
				friendly = false;
				Reset();
			}
			...
			{
				if (!friendly)
					events.ScheduleEvent(EVENT_PLAYER_CHECK, 500);
				else
					DoCast(70339);
			...
			void UpdateAI(const uint32 diff)
			{
				if (me->GetInstanceScript()->GetBossState(DATA_GUNSHIP_EVENT) != IN_PROGRESS)
					me->DespawnOrUnsummon();
				
				if (UpdateVictim())
				{
					if ((!friendly) == (me->getVictim()->GetPositionY() > 2033.0f && me->getVictim()->GetPositionY() < 2437.0f))
						me->getThreatManager().modifyThreatPercent(me->getVictim(), -100);
					
					else if (!me->getVictim()->IsWithinDistInMap(me, 10.0f, false))
					{
						if (!ThrowEventScheduled)
						{
							ThrowEventScheduled = true;
							DoCastVictim(SPELL_RENDING_THROW);
							DoCast(me, SPELL_BATTLE_FURY);
							events.ScheduleEvent(EVENT_RENDING_THROW, 4000);
						}
					}
				}
				...
				if (!me->HasAura(SPELL_TASTE_OF_BLOOD) && !HealthAbovePct(25))
								...
								for(Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
									if (Player* pPlayer = itr->getSource())
										if (pPlayer->isAlive() && !pPlayer->isGameMaster())
											if (pPlayer->GetPositionZ() > 450.0f)
												if (pPlayer->GetPositionY() < 2033.0f || pPlayer->GetPositionY() > 2437.0f)
												{
													found = true;
				...
				if (CanCleave)
				{
					...
					CanCleave = false;
					events.ScheduleEvent(EVENT_CLEAVE, urand(2000, 8000));
				}
			...
			private:
				EventMap events;
				bool CanCleave;
				bool ThrowEventScheduled;
				bool friendly;
		};

		CreatureAI* GetAI(Creature* pCreature) const
		{
			return new npc_commanderAI(pCreature);
		}
};
