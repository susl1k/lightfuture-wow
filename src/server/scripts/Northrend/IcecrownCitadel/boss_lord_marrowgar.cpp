	...
	public:
		...
		struct boss_lord_marrowgarAI : public BossAI
		{
					...
					switch (eventId)
					{
						...
						case EVENT_COLDFLAME:
						{
							...
							if (!me->HasAura(SPELL_BONE_STORM))
							{
								...
								if (target)
								{
									SetGUID(target->GetGUID(), DATA_COLDFLAME_GUID);
									DoCast(target, SPELL_COLDFLAME_NORMAL);
								}
								events.ScheduleEvent(EVENT_COLDFLAME, 5000, EVENT_GROUP_SPECIAL);
							}
							else
						...
						case EVENT_BONE_STORM_END:
							...
							events.ScheduleEvent(EVENT_ENABLE_BONE_SLICE, 10000);
							events.RescheduleEvent(EVENT_COLDFLAME, 5000, EVENT_GROUP_SPECIAL);
							if (!IsHeroic())
								events.RescheduleEvent(EVENT_BONE_SPIKE_GRAVEYARD, 18000, EVENT_GROUP_SPECIAL);
			...
			void MovementInform(uint32 type, uint32 id)
			{
				...
				if (id == POINT_TARGET_BONESTORM_PLAYER && me->HasAura(SPELL_BONE_STORM)) {
					events.ScheduleEvent(EVENT_COLDFLAME, 200, EVENT_GROUP_SPECIAL);
					// lock movement
			...
			void IsSummonedBy(Unit* owner)
			{
				...
				if (owner->HasAura(SPELL_BONE_STORM))
				{
					if (MarrowgarAI* marrowgarAI = CAST_AI(MarrowgarAI, owner->GetAI()))
					{
						...
						//me->GetPosition(&pos);
						owner->GetNearPosition(pos, 0.2f, ang - owner->GetOrientation());
					}
				}
				else
				{
					...
					owner->GetNearPosition(pos, 9.0f, me->GetOrientation() - owner->GetOrientation());
				}
				me->NearTeleportTo(pos.GetPositionX(), pos.GetPositionY(), me->GetPositionZ(), me->GetOrientation());
				_events.ScheduleEvent(EVENT_COLDFLAME_TRIGGER, 800);
			}
...
class spell_marrowgar_coldflame : public SpellScriptLoader
{
	public:
		...
		class spell_marrowgar_coldflame_SpellScript : public SpellScript
		{
			...
			{
				targets.clear();
				Unit* marrowgar=GetCaster();
				Unit* target = ObjectAccessor::GetUnit(*marrowgar, marrowgar->GetAI()->GetGUID(DATA_COLDFLAME_GUID));
			...
			void HandleSpikes(SpellEffIndex effIndex)
			{
				...
				if (Creature* marrowgar = GetCaster()->ToCreature())
				{
					...
					for (uint8 i = 0; i < boneSpikeCount; ++i)
					{
						...
						Unit* target = marrowgarAI->SelectTarget(SELECT_TARGET_RANDOM, 0, TargetSelectorTankPlayerDistancesSpell(marrowgar, true, true, 0.0f, 150.0f, -SPELL_IMPALED));
						if (!target)
							break;
...
class spell_marrowgar_bone_storm : public SpellScriptLoader
{
	public:
		...
		class spell_marrowgar_bone_storm_SpellScript : public SpellScript
		{
			...
			{
				SetHitDamage(int32(GetHitDamage() * (5.0f/(GetHitUnit()->GetExactDist2d(GetCaster())+5.0f))));// DMG * 5 / (distance+5)
			}
