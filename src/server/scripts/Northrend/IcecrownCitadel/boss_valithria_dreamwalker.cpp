...
enum Events
{
	...
	// The Lich King
	EVENT_GLUTTONOUS_ABOMINATION            = 5,
	EVENT_SUPPRESSER                        = 6,
	EVENT_BLISTERING_ZOMBIE                 = 7,
	EVENT_RISEN_ARCHMAGE                    = 8,
	EVENT_BLAZING_SKELETON                  = 9,
		...
		struct boss_valithria_dreamwalkerAI : public ScriptedAI
		{
			...
			void JustSummoned(Creature* summon)
			{
				if (summon->GetEntry() == NPC_DREAM_PORTAL_PRE_EFFECT)
				{
					...
					while(summon->GetPositionX()>4190.0f && summon->GetPositionX()<4219.0f && (summon->GetPositionY()>2417.0f || summon->GetPositionY()<2451.0f) || me->GetExactDist2d(summon->GetPositionX(), summon->GetPositionY())<10.0f) {
						summon->NearTeleportTo(me->GetPositionX()+20.0f-float(rand_norm())*40.0f, me->GetPositionY()+20.0f-float(rand_norm())*40.0f, summon->GetPositionZ(), summon->GetOrientation());
					}
				}
...
const Position MobSpawn[4] =
{
	{ 4241.3398f, 2411.5200f, 364.9519f, 0 },
	{ 4241.3398f, 2557.4899f, 364.9519f, 0 },
	{ 4166.9501f, 2411.5200f, 364.9519f, 0 },
	{ 4166.9501f, 2557.4899f, 364.9519f, 0 },
};
...
class npc_the_lich_king_controller : public CreatureScript
{
	public:
		...
		struct npc_the_lich_king_controllerAI : public ScriptedAI
		{
			...
			{
				_events.Reset();
				_events.ScheduleEvent(EVENT_GLUTTONOUS_ABOMINATION, 5000);
				_events.ScheduleEvent(EVENT_SUPPRESSER, 15000);
				_events.ScheduleEvent(EVENT_BLISTERING_ZOMBIE, 25000);
				_events.ScheduleEvent(EVENT_RISEN_ARCHMAGE, 35000);
				_events.ScheduleEvent(EVENT_BLAZING_SKELETON, 50000);
				me->SetReactState(REACT_PASSIVE);
				entrances = RAID_MODE(1, 3, 1, 3);//-1 due to indexing
				last = urand(0, entrances);
			}
			...
			void JustSummoned(Creature* summon)
			{
				...
				Unit* target;
				if (summon->GetEntry() != NPC_RISEN_ARCHMAGE)
					target = Unit::GetUnit(*me, _instance->GetData64(DATA_VALITHRIA_DREAMWALKER));
				else
			...
			void DoAction(int32 const action)
			{
				uint8 rand = urand(0, entrances);
				while (last == rand)
					rand = urand(0, entrances);
				last = rand;
				Position pos = MobSpawn[rand];
				switch (action)
				{
					case EVENT_GLUTTONOUS_ABOMINATION:
						me->SummonCreature(NPC_GLUTTONOUS_ABOMINATION, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
					break;
					case EVENT_SUPPRESSER:
						for (int i = 0; i < 5; i++)
							me->SummonCreature(NPC_SUPPRESSER, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
					break;
					case EVENT_BLISTERING_ZOMBIE:
						me->SummonCreature(NPC_BLISTERING_ZOMBIE, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
					break;
					case EVENT_RISEN_ARCHMAGE:
						me->SummonCreature(NPC_RISEN_ARCHMAGE, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
					break;
					case EVENT_BLAZING_SKELETON:
						me->SummonCreature(NPC_BLAZING_SKELETON, pos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
					break;
				}
			}
			...
			void UpdateAI(uint32 const diff)
			{
				...
				while (uint32 eventId = _events.ExecuteEvent())
				{
					...
					{
						case EVENT_GLUTTONOUS_ABOMINATION:
							DoAction(EVENT_GLUTTONOUS_ABOMINATION);
							_events.ScheduleEvent(EVENT_GLUTTONOUS_ABOMINATION, Is25ManRaid() ? 50000 : 60000);
						break;
						case EVENT_SUPPRESSER:
							DoAction(EVENT_SUPPRESSER);
							_events.ScheduleEvent(EVENT_SUPPRESSER, Is25ManRaid() ? 50000 : 60000);
						break;
						case EVENT_BLISTERING_ZOMBIE:
							DoAction(EVENT_BLISTERING_ZOMBIE);
							_events.ScheduleEvent(EVENT_BLISTERING_ZOMBIE, Is25ManRaid() ? 25000 : 30000);
						break;
						case EVENT_RISEN_ARCHMAGE:
							DoAction(EVENT_RISEN_ARCHMAGE);
							_events.ScheduleEvent(EVENT_RISEN_ARCHMAGE, Is25ManRaid() ? 25000 : 30000);
						break;
						case EVENT_BLAZING_SKELETON:
							DoAction(EVENT_BLAZING_SKELETON);
							_events.ScheduleEvent(EVENT_BLAZING_SKELETON, Is25ManRaid() ? 50000 : 60000);
						break;
					}
				}
			}
				...
				while (uint32 eventId = _events.ExecuteEvent())
				{
					switch (eventId)
					{
						case EVENT_CHECK_PLAYER:
						{
							...
							_events.ScheduleEvent(player ? EVENT_EXPLODE : EVENT_CHECK_PLAYER, 500);
							break;
						}
						case EVENT_EXPLODE:
