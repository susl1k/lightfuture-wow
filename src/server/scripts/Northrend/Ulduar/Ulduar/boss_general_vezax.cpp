	...
	struct mob_saronit_vaporAI : public ScriptedAI
	{
		...
		void DamageTaken(Unit* /*attacker*/, uint32 &damage)
		{
			if (damage >= me->GetHealth())
			{
				...
				if (!me->HasAura(SPELL_SARONIT_VAPOR))
				{
					me->RemoveAllAuras();
		...
		class spell_general_vezax_saronite_vapors_AuraScript : public AuraScript
		{
			...
			void HandleEffectApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
			{
				if (Unit* caster = GetCaster())
				{
					...
					//caster->CastCustomSpell(GetTarget(), SPELL_SARONITE_VAPORS_ENERGIZE, &mana, NULL, NULL, true, NULL, NULL, GetTarget()->GetGUID());
					//GetTarget()->CastCustomSpell(GetTarget(), SPELL_SARONITE_VAPORS_ENERGIZE, &mana, NULL, NULL, true, NULL, NULL, caster->GetGUID());
