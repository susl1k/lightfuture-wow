            ...
            void ActiveTower()
            {
                if (ActiveTowers)
                {
                    if (towerOfStorms)
                    {
                        ...
                        events.ScheduleEvent(EVENT_THORIM_S_HAMMER, 1*IN_MILLISECONDS);
                    }

                    if (towerOfFlames)
                    {
                        me->AddAura(SPELL_BUFF_TOWER_OF_FLAMES, me);
                        events.ScheduleEvent(EVENT_MIMIRON_S_INFERNO, 1*IN_MILLISECONDS);
                    }

                    if (towerOfFrost)
                    {
                        me->AddAura(SPELL_BUFF_TOWER_OF_FR0ST, me);
                        events.ScheduleEvent(EVENT_HODIR_S_FURY, 1*IN_MILLISECONDS);
                    }

                    if (towerOfLife)
                    {
                        me->AddAura(SPELL_BUFF_TOWER_OF_LIFE, me);
                        events.ScheduleEvent(EVENT_FREYA_S_WARD, 1*IN_MILLISECONDS);
                    }
...
//npc lore keeper
#define GOSSIP_ITEM_1  "Don't Activate Hard Mode"
#define GOSSIP_ITEM_2  "Activate Hard Mode"
            ...
            void DoAction(int32 const action)
            {
                ...
                if (action == 0)
                {
                    for (int32 i = 0; i < RAID_MODE(2, 5); ++i)
                        DoSummon(VEHICLE_SIEGE, PosSiege[i%5], 3000, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                    for (int32 i = 0; i < RAID_MODE(2, 5); ++i)
    ...
    public:
        ...
        class spell_load_into_catapult_AuraScript : public AuraScript
        {
            ...
            void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (owner)
					if (Unit* seat = owner->GetVehicleBase())
						if (Unit* base = seat->GetVehicleBase())
						{
							base->CastSpell(base, SPELL_PASSENGER_LOADED, true);
							owner->ExitVehicle();
							owner->EnterVehicle(base, 3);
							return;
						}
...
enum ThrowPassengerSpells
    {
        SPELL_VEHICLE_PASSENGER_LOADED = 62340,
        SPELL_VEHICLE_LOAD_INTO_CATAPULT = 64414,
    };
        ...
        class spell_vehicle_throw_passenger_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex effIndex)
            {
				if (Unit* caster = GetCaster())
					...
					{
						if (Unit* throwed = vehicle->GetPassenger(3))
						{
							throwed->RemoveAurasDueToSpell(SPELL_VEHICLE_LOAD_INTO_CATAPULT);
							throwed->ExitVehicle();
							
							SpellCastTargets targets = GetSpell()->m_targets;
							float x, y, z;
							targets.GetDstPos()->GetPosition(x, y, z);
							throwed->GetMotionMaster()->MoveJump(x, y, z, targets.GetSpeedXY(), targets.GetSpeedZ());
						}
						vehicle->GetBase()->RemoveAurasDueToSpell(SPELL_VEHICLE_PASSENGER_LOADED);
					}
            }

            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_vehicle_throw_passenger_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };
