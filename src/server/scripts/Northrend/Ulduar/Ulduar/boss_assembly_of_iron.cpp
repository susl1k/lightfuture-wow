...
enum Events
{
	...
	EVENT_OVERWHELMING_POWER_TARGET_CHECK,
	// Molgeim
	EVENT_RUNE_OF_POWER,
	...
	boss_steelbreaker() : CreatureScript("boss_steelbreaker") { }
		...
		struct boss_steelbreakerAI : public BossAI
		{
			...
			void DoAction(int32 const action)
			{
				switch (action)
				{
					case EVENT_UPDATEPHASE:
						...
						{
							me->ResetLootMode();
							events.RescheduleEvent(EVENT_OVERWHELMING_POWER, 3000);
						}
					break;
				}
			}
			...
			void KilledUnit(Unit* who)
			{
				...
				if (_phase >= 3)
					me->CastSpell(me, SPELL_ELECTRICAL_CHARGE_TRIGGERED, false);
			}
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				switch(eventId)
				{
					...
					case EVENT_STATIC_DISRUPTION:
						{
							...
							events.ScheduleEvent(EVENT_STATIC_DISRUPTION, 15000);
						break;
						}
					case EVENT_OVERWHELMING_POWER:
						...
						Overwhelming_PowerGUID = me->getVictim()->GetGUID();
						events.ScheduleEvent(EVENT_OVERWHELMING_POWER, RAID_MODE(62000, 37000));
						events.ScheduleEvent(EVENT_OVERWHELMING_POWER_TARGET_CHECK, RAID_MODE(59800, 34800));
						break;
					case EVENT_OVERWHELMING_POWER_TARGET_CHECK:
						if (Unit* target = Unit::GetUnit(*me, Overwhelming_PowerGUID))
							if (target->isAlive())
								me->CastSpell(me, SPELL_ELECTRICAL_CHARGE_TRIGGERED, false);
						break;
				}
	...
	public:
		...
		class spell_steelbreaker_static_disruption_SpellScript : public SpellScript
		{
			...
			void HandleTriggerMissile(SpellEffIndex effIndex)
			{
				...
				if (caster && target)
				{
					uint32 id = uint32(caster->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL ? 44008 : 63494);
		...
		void JustDied(Unit* /*killer*/)
		{
			...
			if (Creature* Steelbreaker = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_STEELBREAKER)))
				if (Steelbreaker->isAlive())
					Steelbreaker->AI()->DoAction(EVENT_UPDATEPHASE);
		}
				...
				switch(eventId)
				{
					...
					case EVENT_RUNE_OF_POWER:
					{
						...
						if (!target->isAlive() || !target)
							target = me;
						target->CastSpell(target, SPELL_SUMMON_RUNE_OF_POWER);
						events.ScheduleEvent(EVENT_RUNE_OF_POWER, 30000);
					}
					break;
	...
	boss_stormcaller_brundir() : CreatureScript("boss_stormcaller_brundir") { }
		...
		struct boss_stormcaller_brundirAI : public BossAI
		{
			...
			void Reset()
			{
				...
				{
					RespawnEncounter(instance, me);
					instance->SetData(DATA_CANT_DO_THAT, 0);
				}
			...
			{
				if (spell->Id == sSpellMgr->GetSpellIdForDifficulty(SPELL_CHAIN_LIGHTNING_N, me) || spell->Id == sSpellMgr->GetSpellIdForDifficulty(SPELL_LIGHTNING_WHIRL, me))
					instance->SetData(DATA_CANT_DO_THAT, 1);
			}
		...
		void DoAction(int32 const action)
		{
			switch (action)
			{
				case EVENT_UPDATEPHASE:
					...
					me->AddAura(SPELL_SUPERCHARGE, me);
					events.SetPhase(++_phase);
					events.RescheduleEvent(EVENT_CHAIN_LIGHTNING, urand(7000, 12000), 1);
					events.RescheduleEvent(EVENT_OVERLOAD, urand(40000, 50000), 1);
					if (_phase >= 2)
				...
				switch (id)
				{
					...
					case POINT_CHASE:
					{
						if (Unit* target = me->getVictim())
							me->GetMotionMaster()->MovePoint(POINT_CHASE, target->GetPositionX(), target->GetPositionY(), 430.0f);
						break;
					}
				}
			}
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				switch(eventId)
				{
					...
					case EVENT_LIGHTNING_TENDRILS_START:
						...
						me->SendMovementFlagUpdate();
						me->GetMotionMaster()->MovePoint(POINT_FLY, me->GetPositionX(), me->GetPositionY(), 430.0f);
