...
public:
	...
	struct boss_vx_001AI : public BossAI
	{
		...
		void EnterCombat(Unit * /*who*/)
		{
			...
			events.ScheduleEvent(EVENT_ROCKET_STRIKE, 30000);
			...
			if (spinning)
			{
				if (spinTimer <= diff)
				{
					if (Creature* leviathan = me->GetVehicleCreatureBase())
					{
						...
						if (Unit* AerialUnit = Unit::GetUnit(*me, instance->GetData64(DATA_AERIAL_UNIT)))
							AerialUnit->SetOrientation(orient + (direction ? M_PI/60 : -M_PI/60));
					}
					else
					{
					...
					switch(eventId)
					{
						...
						case EVENT_LASER_BARRAGE:
							...
							if (Creature* leviathan = me->GetVehicleCreatureBase())
							{
								...
								if (Unit* AerialUnit = Unit::GetUnit(*me, instance->GetData64(DATA_AERIAL_UNIT)))
									AerialUnit->ToCreature()->SetReactState(REACT_PASSIVE);
								float orient = float(2*M_PI * rand_norm());
								//leviathan->AddAura(42716, leviathan);
								//leviathan->AI()->DoCast(SPELL_SPINNING_UP);
						...
						case EVENT_LASER_BARRAGE_END:
							if (Creature* leviathan = me->GetVehicleCreatureBase())
							{
								//leviathan->RemoveAurasDueToSpell(42716);//Selfroot
								leviathan->SetReactState(REACT_AGGRESSIVE);
								if (leviathan->getVictim())
									leviathan->AI()->AttackStart(leviathan->getVictim());
								if (Unit* AerialUnit = Unit::GetUnit(*me, instance->GetData64(DATA_AERIAL_UNIT)))
								{
									AerialUnit->ToCreature()->SetReactState(REACT_AGGRESSIVE);
									AerialUnit->GetAI()->AttackStart(AerialUnit->getVictim());
								}
							}
							me->SetReactState(REACT_AGGRESSIVE);
							if (me->getVictim())
		...
		void DoAction(const int32 action)
		{
			switch (action)
			{
				...
				case DO_DISABLE_AERIAL:
					...
					me->AddAura(SPELL_MAGNETIC_CORE, me);
