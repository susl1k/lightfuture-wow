		...
		void ReceiveEmote(Player* player, uint32 emote)
		{
			if(m_Phase >= PHASE_BRAIN)
			{
				if(emote == TEXT_EMOTE_KISS)
				{
					...
					if (me->IsWithinLOS(player->GetPositionX(),player->GetPositionY(),player->GetPositionZ()) && me->IsWithinDistInMap(player, 50.0f))
					{
		...
		void SpellHitTarget(Unit* target, const SpellInfo* spell)
		{
			if(target && target->ToPlayer())
			{
				...
				case SPELL_PSYCHOSIS:
					ModifySanity(target->ToPlayer(),RAID_MODE(-12, -9));
					break;
...
public:
	...
	struct npc_ominous_cloudAI : public ScriptedAI
	{
		...
		{
			c->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
			c->ApplySpellImmune(0, IMMUNITY_ID, SPELL_SHADOW_NOVA, true);
			m_pInstance = c->GetInstanceScript();
		}
...
public:
	...
	struct boss_yogg_saronAI : public Scripted_NoMovementAI
	{
		boss_yogg_saronAI(Creature *c) : Scripted_NoMovementAI(c)
		{
			...
			me->SetUnitMovementFlags(MOVEMENTFLAG_SWIMMING);
		}
...
public:
	...
	struct npc_sanity_wellAI : public Scripted_NoMovementAI
	{
		...
		void UpdateAI(const uint32 diff)
		{
			...
			{
				std::list<Unit*> plrList;
				SelectTargetList(plrList, RAID_MODE(10,25), SELECT_TARGET_NEAREST, 10.0f, true, SPELL_SANITY_WELL_DEBUFF);
				for (std::list<Unit*>::iterator itr = plrList.begin(); itr != plrList.end(); ++itr)
				{
					if(Creature* sara = Creature::GetCreature((*me),m_pInstance->GetData64(TYPE_SARA)))
						CAST_AI(boss_sara::boss_saraAI, sara->AI())->ModifySanity((*itr)->ToPlayer(),10);
				}
				uiSanityTick_Timer = 2000;
	...
	struct npc_keeper_helpAI : public Scripted_NoMovementAI
	{
		...
		void UpdateAI(const uint32 diff)
		{
			if(m_pInstance)
			{
				...
				{
					case NPC_HELP_KEEPER_FREYA:
						me->SetVisible(m_pInstance->GetData(TYPE_FREYA) ? true : false/*m_pInstance->GetBossState(TYPE_FREYA) == DONE*/);
					break;
					case NPC_HELP_KEEPER_MIMIRON:
