...
public:
    ...
    bool OnGossipHello(Player *player, GameObject *pGO)
    {
        ...
        {
            if (pInstance->GetData(TYPE_COLOSSUS) == 2 || player->isGameMaster()) //count of 2 collossus death
            ...
            if (pInstance->GetBossState(TYPE_LEVIATHAN) == DONE || player->isGameMaster())
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Teleport to the Colossal Forge", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + FORGE);
            if (pInstance->GetBossState(TYPE_XT002) == DONE || player->isGameMaster())
            {
            ...
            if (pInstance->GetBossState(TYPE_KOLOGARN) == DONE || player->isGameMaster())
            ...
            if (pInstance->GetBossState(TYPE_AURIAYA) == DONE || player->isGameMaster())
            ...
            if (/*pInstance->GetBossState(TYPE_FREYA) == DONE*/pInstance->GetData(TYPE_FREYA) || player->isGameMaster())
            ...
            if (pInstance->GetBossState(TYPE_VEZAX) == DONE || player->isGameMaster())
        ...
        player->SEND_GOSSIP_MENU(pGO->GetGOInfo()->GetGossipMenuId(), pGO->GetGUID());
        return true;
    }
