...
enum AchievementData
{
	...
	ACHIEVEMENT_SHATTERED_N = 2925,
	ACHIEVEMENT_SHATTERED_H = 2926,
};
...
public:
	...
	struct boss_ignis_AI : public BossAI
	{
		...
		void JustDied(Unit* /*victim*/)
		{
			...
			if (instance)
				if (_shattered)
					instance->DoCompleteAchievement(RAID_MODE(ACHIEVEMENT_SHATTERED_N, ACHIEVEMENT_SHATTERED_H));
		}
				...
				switch(eventId)
				{
					...
					case EVENT_GRAB_POT:
						if (Unit* slagPotTarget = Unit::GetUnit(*me, _slagPotGUID))
						{
							...
							events.ScheduleEvent(EVENT_CHANGE_POT, 10000);
						}
					break;
					...
					case EVENT_CONSTRUCT:
					{
						...
						if (!_creatureList.empty())
						{
							...
							if (Creature* construct = (*itr))
							{
								...
								construct->ApplySpellImmune(0, IMMUNITY_ID, 62343, false);
...
public:
	...
	struct npc_iron_constructAI : public ScriptedAI
	{
		...
		void Reset()
		{
			...
			_brittled = false;
			me->ApplySpellImmune(0, IMMUNITY_ID, 62343, true);
		}
