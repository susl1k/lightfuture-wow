...
#define SPELL_SQUEEZED_LIFELESS 64708
...
#define EMOTE_EYEBEAM           "Kologarn focuses his attention on you!"
...
enum
{
	...
	ACHIEVEMENT_IF_LOOK_COULD_KILL_N = 2955,
	ACHIEVEMENT_IF_LOOK_COULD_KILL_H = 2956,

	DATA_RUBBLE_AND_ROLL                        = 1,
	DATA_DISARMED                               = 2,
	DATA_WITH_OPEN_ARMS                         = 3, 
	DATA_IF_LOOK_COULD_KILL                     = 4,
	GUID_FOCUS = 5,
	EMENGERCY_EXIT = 6,
};
...
class boss_kologarn : public CreatureScript
{
	public:
		...
		{
			boss_kologarnAI(Creature* creature) : BossAI(creature, TYPE_KOLOGARN), vehicle(creature->GetVehicleKit()), left(false), right(false)
			{
				ASSERT(vehicle);
			...
			void EnterCombat(Unit* /*who*/)
			{
				...
				instance->DoUseDoorOrButton(instance->GetData64(DATA_KOLOGARN_DOOR));
			...
			void EnterEvadeMode()
			{
				...
				for (uint8 i = 0; i < 2; ++i)
					if (Unit* arm = vehicle->GetPassenger(i))
						arm->ToCreature()->AI()->EnterEvadeMode();
			...
			void JustDied(Unit* /*victim*/)
			{
				...
				for (uint8 i = 0; i < 2; ++i)
					if (Unit* arm = vehicle->GetPassenger(i))
					{
						if (arm->GetEntry() == NPC_RIGHT_ARM)
							arm->GetAI()->DoAction(EMENGERCY_EXIT);
						arm->ExitVehicle();
					}
				...
				if (IsLookCouldKill)
					instance->DoCompleteAchievement(Is25ManRaid() ? ACHIEVEMENT_IF_LOOK_COULD_KILL_H : ACHIEVEMENT_IF_LOOK_COULD_KILL_N);
			}
			...
			void PassengerBoarded(Unit* who, int8 /*seatId*/, bool apply)
			{
				...
				if (!apply)
				{
					...
					float x = 1785.0f, 
					y = who->GetEntry() == NPC_LEFT_ARM ? -47.0f : -5.0f, 
					z = 449.0f, 
					o = 0.0f;
					for (uint8 i = 0; i < 5; i++)
						me->SummonCreature(NPC_RUBBLE, x, y, z, o);
			...
			{
				if (type == DATA_IF_LOOK_COULD_KILL)
					IsLookCouldKill = false;
			}
			...
			void UpdateAI(uint32 const diff)
			{
				...
				switch (events.GetEvent())
				{
					...
					case EVENT_STONE_GRIP:
					{
						if (right)
						{
							Unit* rightArm = Unit::GetUnit(*me, instance->GetData64(DATA_RIGHT_ARM));
							rightArm->GetAI()->DoAction(EVENT_STONE_GRIP);
		...
		void DoAction(int32 const action)
		{
			switch (action)
			{
				case EVENT_STONE_GRIP:
				{
					...
					kologarn->AI()->SelectTargetList(targetList, TargetSelectorTankPlayerDistancesSpell(kologarn, true, true), RAID_MODE(1,3), SELECT_TARGET_RANDOM);
					if (!targetList.empty())
					{
						absorbing = true;
						absorb = Is25ManRaid() ? 480000 : 100000;
						for (std::list<Unit*>::iterator i = targetList.begin(); i != targetList.end(); i++)
						{
							kologarn->AddAura(SPELL_STONE_GRIP_DOT, *i);
							(*i)->EnterVehicle(me);
						}
					}
					break;
				...
				case EMENGERCY_EXIT:
				{
					...
					for (uint8 i = 0; i < RAID_MODE(1,3); ++i)
						if (Unit* unit = me->GetVehicleKit()->GetPassenger(i))
						{
							unit->RemoveAurasDueToSpell(SPELL_STONE_GRIP_DOT);
							unit->RemoveAurasDueToSpell(SPELL_SQUEEZED_LIFELESS);
							unit->ExitVehicle();
							unit->ClearUnitState(UNIT_STATE_ONVEHICLE);
							unit->ToPlayer()->TeleportTo(603, 1756.25f + irand(-3, 3), -8.3f + irand(-3, 3), 448.8f, 0.0f);
							//player->GetMotionMaster()->MoveJump(1756.25f + irand(-3, 3), -8.3f + irand(-3, 3), 448.8f, 30.0f, 10.0f);
						}
					break;
				}
			}
...
public:
	...
	struct npc_focused_eyebeamAI : public ScriptedAI
	{
		...
		{
			targetGUID = summoner->GetAI()->GetGUID(GUID_FOCUS);
		}
		
		void DamageDealt(Unit* /*target*/, uint32& /*damage*/, DamageEffectType /*damageType*/)
		{
			if (damaging)
				me->ToTempSummon()->GetSummoner()->GetAI()->SetData(DATA_IF_LOOK_COULD_KILL, 0);
		}
	...
	public:
		...
		class spell_ulduar_stone_grip_cast_target_SpellScript : public SpellScript
		{
			...
			void HandleForceCast(SpellEffIndex i)
			{
				...
				plr->CastSpell(GetExplTargetUnit(), GetSpellInfo()->Effects[i].TriggerSpell, true);
				PreventHitEffect(i);
			}
...
public:
	...
	class spell_ulduar_squeezed_lifeless_SpellScript : public SpellScript
	{
		...
		void MoveCorpse()
		{
			...
			GetHitUnit()->ToPlayer()->TeleportTo(603, 1756.25f + irand(-3, 3), -8.3f + irand(-3, 3), 448.8f, 0.0f);
			//GetHitUnit()->GetMotionMaster()->MoveJump(1756.25f + irand(-3, 3), -8.3f + irand(-3, 3), 449.0f, 30, 10);
		}
