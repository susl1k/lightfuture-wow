...
#define ACHIEVEMENT_DONT_STAND_IN_LIGHTNING RAID_MODE(2971, 2972)
	...
	struct boss_thorimAI : public BossAI
	{
		...
		void EncounterIsDone()
		{
			...
			if (instance)
			{
				...
				if (DontStandInLightning)
					instance->DoCompleteAchievement(ACHIEVEMENT_DONT_STAND_IN_LIGHTNING);
		...
		void EnterCombat(Unit* /*who*/)
		{
			...
			*/
			...
			if (GameObject* go = ObjectAccessor::GetGameObject(*me, instance->GetData64(DATA_THORIM_LEVER)))
			{
		...
		{
			if (spell->Id == SPELL_LIGHTNING_RELEASE)
				if (target->GetTypeId() == TYPEID_PLAYER)
					DontStandInLightning = false;
		}
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch(eventId)
					{
						...
						case EVENT_STORMHAMMER:
							if (Creature* source = me->SummonCreature(34055, 2134.79f, -263.03f, 419.84f, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 1500))
								source->CastSpell(source, SPELL_STORMHAMMER, true, NULL, NULL, me->GetGUID());
							events.ScheduleEvent(EVENT_STORMHAMMER, urand(15000, 20000), 0, PHASE_1);
							break;
						case EVENT_CHARGE_ORB:
							if (Creature* source = me->SummonCreature(34055, PosOrbs[urand(0, 6)], TEMPSUMMON_TIMED_DESPAWN, 16000))
								me->AddAura(SPELL_CHARGE_ORB, source);
				...
				while (uint32 eventId = events.ExecuteEvent())
				{
					switch(eventId)
					{
						...
						case EVENT_TRANSFER_ENERGY:
							if (Creature* source = me->SummonCreature(34055, PosCharge[urand(0, 6)], TEMPSUMMON_TIMED_DESPAWN, 9000))
						...
						case EVENT_RELEASE_ENERGY:
							if (Creature* source = me->FindNearestCreature(34055, 100.0f))
...
public:
	...
	struct npc_ancient_rune_giantAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 const diff)
		{
			...
			if (FortificationTimer <= diff)
			{
				std::list<Creature*> lst = DoFindFriendlyMissingBuff(20.0f, SPELL_RUNIC_FORTIFICATION);
				std::list<Creature*>::const_iterator itr = lst.begin();
				if (!lst.empty())
				{
					advance(itr, rand()%lst.size());
					me->AddAura(SPELL_RUNIC_FORTIFICATION, (*itr));
				}
				FortificationTimer = urand(10000, 12000);
			}
			else FortificationTimer -= diff;
...
public:
	...
	bool OnGossipHello(Player* player, GameObject* go)
	{
		...
		instance->DoUseDoorOrButton(instance->GetData64(DATA_THORIM_HALLWAY_DOOR), 10);
		instance->DoUseDoorOrButton(go->GetGUID(), 10);
		return true;
	}
};
