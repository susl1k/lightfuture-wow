	...
	struct boss_freyaAI : public ScriptedAI
	{
		...
		void EncounterIsDone()
		{
			...
			for (uint8 n = 0; n < 3; ++n)
			{
				Elder[n] = ObjectAccessor::GetCreature(*me, pInstance->GetData64(TYPE_ELDER_BRIGHTLEAF + n));
				if (Elder[n] && Elder[n]->isAlive())
					Elder[n]->DespawnOrUnsummon(5000);
			}
		}
				...
				if(uiNaturalBomb_Timer <= diff)
				{
					...
					for (std::list<HostileReference*>::const_iterator itr = threatlist.begin(); itr != threatlist.end(); ++itr)
					{
						if ((*itr)->getTarget()->GetTypeId() != TYPEID_PLAYER)
							continue;
						plrList.push_back((*itr)->getTarget()->ToPlayer());
					}
   ...
   mob_iron_roots() : CreatureScript("mob_iron_roots") { }
	...
	struct mob_iron_rootsAI : public ScriptedAI
	{
		...
		void JustDied(Unit* /*killer*/)
		{
			...
			{
				Roots->RemoveAura(SPELL_IRON_ROOTS_10);
				Roots->RemoveAura(SPELL_IRON_ROOTS_25);
				Roots->RemoveAura(SPELL_FREYA_IRON_ROOTS_10);
				Roots->RemoveAura(SPELL_FREYA_IRON_ROOTS_25);
			}
		...
		void UpdateAI(uint32 const diff)
		{
			if (timer)
			{
				...
				{
					DoZoneInCombat();
					Unit* target = SelectTarget(SELECT_TARGET_NEAREST, 0, 100.0f, true, RAID_MODE(SPELL_IRON_ROOTS_10, SPELL_IRON_ROOTS_25));
					if (!target)
						target = SelectTarget(SELECT_TARGET_NEAREST, 0, 100.0f, true, RAID_MODE(SPELL_FREYA_IRON_ROOTS_10, SPELL_FREYA_IRON_ROOTS_25));
					if (target)
						_RootsGUID = target->GetGUID();
					timer = 0;
				} else timer -= diff;
			}
		}
