...
#define ACHIEVEMENT_STAYING_BUFFED_ALL_WINTER   RAID_MODE(2969, 2970)
		...
		void UpdateAI(uint32 const diff)
		{
			...
			if (uiCheckIntenseColdTimer < diff)
			{
				...
				for (std::list<HostileReference*>::const_iterator itr = ThreatList.begin(); itr != ThreatList.end(); ++itr)
				{
					...
					if (pTarget->HasAura(SPELL_STARLIGHT) && pTarget->HasAura(RAID_MODE(63711, 65134)) && pTarget->HasAura(SPELL_SINGED))
					{
						AchievementEntry const* pAE = sAchievementStore.LookupEntry(ACHIEVEMENT_STAYING_BUFFED_ALL_WINTER);
						pTarget->ToPlayer()->CompletedAchievement(pAE);
					}
			...
			for (std::list<HostileReference*>::const_iterator itr = ThreatList.begin(); itr != ThreatList.end(); ++itr)
			{
				if (Unit *pTarget = Unit::GetUnit(*me, (*itr)->getUnitGuid()))
				{
					if (pTarget->GetTypeId() != TYPEID_PLAYER)
					{
						...
						for (uint8 i = 0; i < RAID_MODE<uint8>(NORMAL_COUNT, RAID_COUNT); ++i)
						{
							if (pTarget->GetEntry() == addLocations[i].entry)
							{
								broken = false;
								break;
							}
...
public:
	...
	struct npc_hodir_priestAI : public ScriptedAI
	{
		...
		{
			if (!UpdateVictim() || me->HasAuraType(SPELL_AURA_MOD_STUN))
				return;
...
public:
	...
	struct npc_hodir_mageAI : public ScriptedAI
	{
		...
		void UpdateAI(const uint32 uiDiff)
		{
			...
			{
				Creature* pShard = me->FindNearestCreature(ENTRY_NPC_FLASH_FREEZE,50,true);
				if (!pShard)
					Creature* pShard = me->FindNearestCreature(ENTRY_NPC_FLASH_FREEZE_PRE,50,true);
				if (pShard)
				{
