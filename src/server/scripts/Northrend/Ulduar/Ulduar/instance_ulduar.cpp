...
public:
	...
	struct instance_ulduar_InstanceMapScript : public InstanceScript
	{
		...
		uint64 uiThorimHallwayDoorGUID;
		...
		uint32 DeforestationTimers[6];
		uint32 TimeRemainingLumerjacked;
		uint32 TimeRemainingConspeedatory;
		uint8 DeadEldersCount;
		bool ConspeedatoryTry;
		bool KilledFreya;
		...
		void Initialize()
		{
			...
			CantDoThatData = true;
			TimeRemainingLumerjacked = 0;
			TimeRemainingConspeedatory = 0;
			DeadEldersCount = 0;
			ConspeedatoryTry = false;
			KilledFreya = false;
			for (uint8 i = 0; i < 6; i++)
				DeforestationTimers[i] = 0;
		...
		void OnGameObjectCreate(GameObject* go)
		{
			switch(go->GetEntry())
			{
				...
				case GO_THORIM_LEVER:
					...
					break;
				case GO_THORIM_HALLWAY_DOOR:
					uiThorimHallwayDoorGUID = go->GetGUID();
					go->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
					break;
			...
			switch (type)
			{
				...
				case TYPE_ASSEMBLY:
					...
					if (state == DONE)
					{
						...
						if (CantDoThatData)
							DoCompleteAchievement(instance->Is25ManRaid() ? ACHIEVEMENT_CANT_DO_THAT_WHILE_STUNNED_H : ACHIEVEMENT_CANT_DO_THAT_WHILE_STUNNED_N);
					}
					break;
				case TYPE_AURIAYA:
					break;
				...
				case TYPE_FREYA:
					if (state == DONE)
					{
						...
						if (GameObject* go = instance->GetGameObject(uiFreyaChestGUID))
							go->SetRespawnTime(go->GetRespawnDelay());
						if (TimeRemainingConspeedatory)
							DoCompleteAchievement(instance->Is25ManRaid() ? ACHIEVMENT_CON_SPEED_ATORY_H : ACHIEVMENT_CON_SPEED_ATORY_N);
					}
					break;
				case TYPE_ALGALON:
					switch (state)
		...
		void SetData(uint32 type, uint32 data)
		{
			switch (type)
			{
				...
				case DATA_CANT_DO_THAT:
					CantDoThatData = data ? false : true;
					break;
				case DATA_CALL_TRAM:
		...
		uint64 GetData64(uint32 data)
		{
			switch (data)
			{
				...
				case DATA_KOLOGARN_DOOR:        return uiKologarnDoorGUID;
				...
				case DATA_THORIM_LEVER:         return uiThorimLeverGUID;
				case DATA_THORIM_HALLWAY_DOOR:  return uiThorimHallwayDoorGUID;
		...
		std::string GetBossSaveData()
		{
			...
			for (int i = 0; i < TYPE_COLOSSUS; ++i)
				saveStream << ((uint32)(uiEncounter[i])) << ' ';
		...
		std::string GetSaveData()
		{
			...
			std::ostringstream saveStream;
			saveStream << "U U " << GetBossSaveData() << GetData(TYPE_COLOSSUS) << " " << uiPlayerDeathFlag << " " << uiAlgalonCountdown << " " << uint32(ConspeedatoryTry);
		...
		void Load(const char* strIn)
		{
			...
			{
				for (uint8 i = 0; i < TYPE_COLOSSUS/*MAX_ENCOUNTER*/; ++i)
				{
					uint32 tmpState;
					loadStream >> tmpState;
					...
					if (i == TYPE_FREYA && tmpState == DONE && !KilledFreya)
						KilledFreya = true;
				}
				uint32 tmpState, tmpState2, tmpState3, tmpState4;
				loadStream >> tmpState >> tmpState2 >> tmpState3 >> tmpState4;
				ColossusData = tmpState;
				uiPlayerDeathFlag = tmpState2;
				uiAlgalonCountdown = tmpState3;
				ConspeedatoryTry = tmpState4 ? true : false;
			}
		...
		void OnUnitDeath(Unit* unit)
		{
			...
			{
				case NPC_CONSPEEDATORY_SERVITOR:
				case NPC_CONSPEEDATORY_NYMPH:
				case NPC_CONSPEEDATORY_GUARDIAN_LASHER:
				case NPC_CONSPEEDATORY_SWARMER:
				case NPC_CONSPEEDATORY_IRONROOT_LASHER:
				case NPC_CONSPEEDATORY_ENT:
				case NPC_CONSPEEDATORY_GUARDIAN:
				case NPC_CONSPEEDATORY_BLADE:
					if (!ConspeedatoryTry)
					{
						ConspeedatoryTry = true;
						TimeRemainingConspeedatory = 1200000;
					}
				break;
				case NPC_ELDER_IRONBRANCH:
				case NPC_ELDER_STONEBARK:
				case NPC_ELDER_BRIGHTLEAF:
					DeadEldersCount++;
					if (DeadEldersCount == 1)
						TimeRemainingLumerjacked = 15000;
					else if (DeadEldersCount == 3 && TimeRemainingLumerjacked && GetBossState(TYPE_FREYA) != DONE)
						DoCompleteAchievement(instance->Is25ManRaid() ? ACHIEVEMENT_LUMBERJACKED_H : ACHIEVEMENT_LUMBERJACKED_N);
				break;
				case NPC_DEFORESTATION_SNAPLASHER:
				case NPC_DEFORESTATION_STORM_LASHER:
				case NPC_DEFORESTATION_ANCIENT_WATER_SPIRIT:
					for (uint8 i = 0; i < 6; i++)
						if (!DeforestationTimers[i])
						{
							DeforestationTimers[i] = 15000;
							break;
						}
					...
					if (!uncomplete)
						DoCompleteAchievement(instance->Is25ManRaid() ? ACHIEVEMENT_DEFORESTATION_H : ACHIEVEMENT_DEFORESTATION_N);
				break;
			}
		...
		void Update(uint32 diff)
		{
			...
			for (uint8 i = 0; i < 6; i++)
			{
				if (DeforestationTimers[i] > diff)
					DeforestationTimers[i] -= diff;
				else DeforestationTimers[i] = 0;
			}
			
			if (TimeRemainingConspeedatory > diff)
				TimeRemainingConspeedatory -= diff;
			else TimeRemainingConspeedatory = 0;
			
			if (TimeRemainingLumerjacked > diff)
				TimeRemainingLumerjacked -= diff;
			else TimeRemainingLumerjacked = 0;
		}
	};
};
