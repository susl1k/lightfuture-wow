	...
	struct boss_auriayaAI : public BossAI
	{
		...
		void JustSummoned(Creature* summoned)
		{
			...
			if (summoned->GetEntry() == NPC_FERAL_DEFENDER)
			{
				...
				summoned->SetAuraStack(SPELL_FERAL_ESSENCE, summoned, _defenderLifeCount);
				_DefenderGUID = summoned->GetGUID();
				DoZoneInCombat(summoned);
			}
		}
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				switch (eventId)
				{
					...
					case EVENT_FERAL_DEFENDER_RESURRECTION:
						if (Creature* defender = ObjectAccessor::GetCreature(*me, _DefenderGUID))
						{
							defender->Respawn();
...
public:
	...
	struct mob_sanctum_sentryAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 const diff)
		{
			...
			if (_pounceTimer <= diff)
			{
				DoCastVictim(RAID_MODE(SPELL_SAVAGE_POUNCE_10, SPELL_SAVAGE_POUNCE_25));
				_pounceTimer = urand(12000, 17000);
			}
			else
				_pounceTimer -= diff;
			DoMeleeAttackIfReady();
