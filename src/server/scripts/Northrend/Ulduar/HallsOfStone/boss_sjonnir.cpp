    ...
    struct mob_malformed_oozeAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (uiMergeTimer <= diff)
            {
				...
				me->GetCreatureListWithEntryInGrid(list, me->GetEntry(), 3.0f);
				for (std::list<Creature*>::iterator i = list.begin(); i != list.end(); i++)
					if (*i != me)
					{
						DoZoneInCombat(me->SummonCreature(CREATURE_IRON_SLUDGE, *me, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000));
						(*i)->DespawnOrUnsummon();
...
public:
    ...
    struct mob_iron_sludgeAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
			if (Creature* Sjonnir = me->FindNearestCreature(CREATURE_SJONNIR, 100))
