...
static Position EyeLocation[] = 
{
	{928.0f, 331.276f, 219.733f, 1.8326f},
	{923.5f, 326.358f, 219.733f, 2.28638f},
	{895.933f, 363.528f, 219.338f, 5.58505f},
	{909.0f, 345.0f, 204.0f, 0.0f}
};
        ...
        void WaypointReached(uint32 waypointId)
        {
            switch (waypointId)
            {
                ...
                case 17:
                    ...
                    if (instance)
						instance->DoUseDoorOrButton(instance->GetData64(DATA_GO_TRIBUNAL_CONSOLE));
           ...
           switch (uiType)
           {
               case 1:
               {
                   ...
                   for (uint8 i = 0; i < uiSpawnNumber; ++i)
					   me->SummonCreature(CREATURE_DARK_RUNE_PROTECTOR, SpawnLocations[random], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
                   me->SummonCreature(CREATURE_DARK_RUNE_STORMCALLER, SpawnLocations[random], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
                   break;
               }
               case 2:
                   for (uint8 i = 0; i < 2; ++i)
                   ...
                   break;
               case 3:
                   me->SummonCreature(CREATURE_IRON_GOLEM_CUSTODIAN, SpawnLocations[random], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
                   break;
           }
         }
            ...
            if (uiPhaseTimer <= uiDiff)
            {
                switch (uiStep)
                {
                    ...
                    case 8:
						...
						{
							kaddrak->UseDoorOrButton();
							me->SummonCreature(CREATURE_KADDRAK, EyeLocation[0]);
							me->SummonCreature(CREATURE_KADDRAK, EyeLocation[1]);
						}
                        JumpToNextStep(5000);
                        break;
                    case 9:
                    ...
                    case 12:
						...
						{
							marnak->UseDoorOrButton();
							me->SummonCreature(CREATURE_MARNAK, EyeLocation[2]);
						}
                    ...
                    case 18:
						...
						{
							abedneum->UseDoorOrButton();
							me->SummonCreature(CREATURE_TRIBUNAL_OF_THE_AGES, EyeLocation[3]);
						}
                        JumpToNextStep(5000);
                        break;
                    case 19:
                    ...
                    case 28:
						...
						if (GameObject* floor = me->FindNearestGameObject(191527, 100))
						{
							floor->UseDoorOrButton();
						}
						
						if (Creature* kaddrak = me->FindNearestCreature(CREATURE_KADDRAK, 100))
							kaddrak->DespawnOrUnsummon();
						if (Creature* kaddrak = me->FindNearestCreature(CREATURE_KADDRAK, 100))
							kaddrak->DespawnOrUnsummon();
						if (Creature* marnak = me->FindNearestCreature(CREATURE_MARNAK, 100))
							marnak->DespawnOrUnsummon();
						if (Creature* abedneum = me->FindNearestCreature(CREATURE_TRIBUNAL_OF_THE_AGES, 100))
							abedneum->DespawnOrUnsummon();
...
public:
	...
	struct npc_dark_matterAI : public NullCreatureAI
    {
		...
		{
			if (type == FOLLOW_MOTION_TYPE && !casted)
			{
				casted = true;
...
public:
	...
	struct npc_searing_gazeAI : public NullCreatureAI
    {
        npc_searing_gazeAI(Creature* creature) : NullCreatureAI(creature) { }

		void Reset()
		{
			DoCastAOE(SPELL_SEARING_GAZE);
		}
    };
