    ...
    public:
        ...
        struct npc_dragonflayer_forge_masterAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* /*killer*/) override
            {
				...
				me->GetCreatureListWithEntryInGrid(list, me->GetEntry(), 100);
				switch (me->GetDBTableGUIDLow())
				{
					case 125904:
						for (std::list<Creature*>::iterator i = list.begin(); i != list.end(); i++)
						{
							if ((*i)->GetPositionY() < -15.0f && (*i)->GetPositionY() > -17.0f)
							{
								(*i)->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_NOT_SELECTABLE);
								break;
							}
						}
						break;
