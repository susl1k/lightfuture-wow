...
class boss_ingvar_the_plunderer : public CreatureScript
{
    public:
        ...
        struct boss_ingvar_the_plundererAI : public BossAI
        {
            ...
            {
				if (damage >= me->GetHealth() && phase == PHASE_HUMAN)
                {
                    me->RemoveAllAuras();
            ...
            {
                if (!UpdateVictim() && phase != PHASE_EVENT)
                    return;

                events.Update(diff);

                if (me->HasUnitState(UNIT_STATE_CASTING) && phase != PHASE_EVENT)
                    return;
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_BANSHEE:
							me->SummonCreature(NPC_ANNHYLDE_THE_CALLER, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 30.0f, me->GetOrientation());
                            return;
                        // PHASE TWO
                        ...
                        case EVENT_SHADOW_AXE:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true))
								if (Creature* axe = me->SummonCreature(23996, *me, TEMPSUMMON_TIMED_DESPAWN, 10000))
								{
									me->AddAura(SPELL_SHADOW_AXE_PERIODIC_DAMAGE, axe);
									axe->GetMotionMaster()->MovePoint(0, *target);
								}
                            events.ScheduleEvent(EVENT_SHADOW_AXE, 30*IN_MILLISECONDS, 0, PHASE_UNDEAD);
                            break;
                        default:
