...
public:
    ...
    {
       boss_marwynAI(Creature *pCreature) : ScriptedAI(pCreature), summons(me), wave(me)
       {
       ...
       void UpdateAI(const uint32 uiDiff)
        {
			...
			{
				Reset();
				if (Unit* falric = Unit::GetUnit(*me, m_pInstance->GetData64(NPC_FALRIC)))
				{
					if (!falric->isAlive())
						falric->ToCreature()->Respawn();
					falric->GetAI()->Reset();
				}
				if (Creature* leader = Unit::GetCreature(*me, m_pInstance->GetData64(DATA_ESCAPE_LIDER)))
				{
					leader->NearTeleportTo(leader->GetHomePosition().GetPositionX(), leader->GetHomePosition().GetPositionY(), leader->GetHomePosition().GetPositionZ(), leader->GetHomePosition().GetOrientation());
					leader->AI()->Reset();
				}
