    ...
    public:
        ...
        struct instance_pit_of_saron_InstanceScript : public InstanceScript
        {
            instance_pit_of_saron_InstanceScript(Map* map) : InstanceScript(map)
            {
				...
				_gauntletDummy1GUID = 0;
				_gauntletDummy2GUID = 0;
				_sindragosaGUID = 0;
				_slaveLeaderGUID = 0;
				_endDoorGUID = 0;
                _teamInInstance = 0;
            ...
            void OnCreatureCreate(Creature* creature)
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_GORKUN_IRONSKULL_1:
						...
						_slaveLeaderGUID = creature->GetGUID();
                        break;
                    case NPC_GORKUN_IRONSKULL_2:
                        if (_teamInInstance == ALLIANCE)
                            creature->UpdateEntry(NPC_MARTIN_VICTUS_2, ALLIANCE);
                        break;
					case NPC_GAUNTLET_DUMMY_1:
						_gauntletDummy1GUID = creature->GetGUID();
						break;
					case NPC_GAUNTLET_DUMMY_2:
						_gauntletDummy2GUID = creature->GetGUID();
						break;
					case NPC_SINDRAGOSA:
                ...
                switch (type)
                {
                    case DATA_GARFROST:
                        if (state == DONE)
                        {
                            ...
                            {
                                if (_teamInInstance == ALLIANCE)
                                    summoner->SummonCreature(NPC_MARTIN_VICTUS_2, SlaveLeaderPos, TEMPSUMMON_MANUAL_DESPAWN);
                                else
            ...
            uint64 GetData64(uint32 type)
            {
                switch (type)
                {
					...
					case DATA_GAUNTLET_DUMMY_1:
						return _gauntletDummy1GUID;
					case DATA_GAUNTLET_DUMMY_2:
						return _gauntletDummy2GUID;
					case DATA_SINDRAGOSA:
						return _sindragosaGUID;
					case DATA_SLAVE_LEADER:
						return _slaveLeaderGUID;
					case DATA_END_DOOR:
						return _endDoorGUID;
                    default:
                        break;
			...
			void Update(uint32 diff)
			{
				...
				if (_introRunning && _introTimer)
				{
					if (_introTimer <= diff)
					{
						Creature* leader = instance->GetCreature(_jainaOrSylvanas1GUID);
						Creature* tyrannus = instance->GetCreature(_tyrannusEventGUID);
						if (leader && tyrannus)
						{
							leader->AI()->DoAction(ACTION_ENTERANCE);
							tyrannus->AI()->DoAction(ACTION_ENTERANCE);
						}
        ...
        private:
			...
			uint64 _gauntletDummy1GUID;
			uint64 _gauntletDummy2GUID;
			uint64 _sindragosaGUID;
			uint64 _slaveLeaderGUID;
			uint64 _endDoorGUID;
