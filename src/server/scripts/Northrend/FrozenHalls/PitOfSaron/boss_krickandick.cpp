...
class boss_ick : public CreatureScript
{
    public:
        ...
        struct boss_ickAI : public BossAI
        {
			...
			Unit* GetPursuitTarget()
			{
				Unit* temp = pursuitTarget;
				pursuitTarget = NULL;
				return temp;
			}
                    ...
                    switch (eventId)
                    {
                        case EVENT_TOXIC_WASTE:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_TOXIC_WASTE, false);
                        ...
                        case EVENT_PURSUIT:
							...
							pursuitTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 200.0f, true);
							me->CastCustomSpell(SPELL_PURSUIT, SPELLVALUE_MAX_TARGETS, 1, NULL, false);
                            break;
                        default:
                            break;
                    }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_OUTRO_6:
							...
							if (Creature* tyrannus = me->SummonCreature(NPC_TYRANNUS_EVENTS, outroPos[1]))
							{

                                tyrannus->SetSpeed(MOVE_FLIGHT, 3.5f, true);
                                tyrannus->GetMotionMaster()->MovePoint(0, outroPos[4]);
                        ...
                        case EVENT_OUTRO_13:
                            ...
                            if (Creature* tyrannus = ObjectAccessor::GetCreature(*me, _tyrannusGUID))
                                tyrannus->GetMotionMaster()->MovePoint(POINT_DESPAWN, outroPos[7]);
                            break;
...
class spell_krick_pursuit : public SpellScriptLoader
{
    public:
        ...
        class spell_krick_pursuit_AuraScript : public AuraScript
        {
			...
			void AddThreat(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
					if (Unit* target = GetTarget())
						if (Creature* creCaster = caster->ToCreature())
						{
							CAST_AI(boss_ick::boss_ickAI, creCaster->AI())->SetTempThreat(caster->getThreatManager().getThreat(target));
							caster->AddThreat(target, 5000000.0f);
            ...
            void Register()
            {
				AfterEffectApply += AuraEffectApplyFn(spell_krick_pursuit_AuraScript::AddThreat, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
