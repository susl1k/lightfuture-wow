                ...
                if (instance)
                {
                    if (TempSummon* summ = me->ToTempSummon())
					{
                        if (Creature* bronjahm = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_BRONJAHM)))
                            bronjahm->CastSpell(bronjahm, SPELL_CONSUME_SOUL, true);
...
class spell_bronjahm_corrupt_soul : public SpellScriptLoader
{
	public:
		spell_bronjahm_corrupt_soul() : SpellScriptLoader("spell_bronjahm_corrupt_soul") { }
		...
		{
			return new spell_bronjahm_corrupt_soul_AuraScript();
		}
};
...
void AddSC_boss_bronjahm()
{
    ...
    new spell_bronjahm_corrupt_soul();
