...
#define HAS_ESSENCE(a) ((a)->HasItemCount(ITEM_EMERALD_ESSENCE, 1, false) || (a)->HasItemCount(ITEM_AMBER_ESSENCE, 1, false) || (a)->HasItemCount(ITEM_RUBY_ESSENCE, 1, false))
...
class spell_shock_lance : public SpellScriptLoader
{
	public:
		spell_shock_lance() : SpellScriptLoader("spell_shock_lance") { }

		class spell_shock_lance_SpellScript : public SpellScript
		{
			PrepareSpellScript(spell_shock_lance_SpellScript);

			void RecalculateDamage()
			{
				Unit* target = GetHitUnit();
				Unit* caster = GetCaster();
				if (Aura* aura = target->GetAura(SPELL_SHOCK_CHARGE, caster->GetGUID()))
				{
					SetHitDamage(int32(GetHitDamage() + 6525 * aura->GetStackAmount()));
					target->RemoveAurasDueToSpell(SPELL_SHOCK_CHARGE, caster->GetGUID());
				}
			}

			void Register()
			{
				OnHit += SpellHitFn(spell_shock_lance_SpellScript::RecalculateDamage);
			}
		};

		SpellScript* GetSpellScript() const
		{
			return new spell_shock_lance_SpellScript();
		}
};

#define NPC_DRAKE 27638
#define SPELL_ICE_BEAM_N 49549
#define SPELL_ICE_BEAM_H 59211
#define SPELL_ARCANE_BOLT_N 51243
#define SPELL_ARCANE_BOLT_H 59215
#define SPELL_TEMPORAL_RIFT 49592
#define SPELL_SHOCK_CHARGE 49836

class npc_drakes_and_whelps : public CreatureScript
{
public:
    npc_drakes_and_whelps() : CreatureScript("npc_drakes_and_whelps") { }

    struct npc_drakes_and_whelpsAI : public ScriptedAI
    {
        npc_drakes_and_whelpsAI(Creature* creature) : ScriptedAI(creature) { }
	
		void Reset()
		{
			abilityTimer = me->GetEntry() == NPC_DRAKE ? urand(4000, 8000) : urand(0, 20);
			for (int i = 0; i < 5; i++)
			{
				drakesGUID[i] = 0;
				drakesDMG[i] = 0;
		...
		{
			if (spell->Id == SPELL_TEMPORAL_RIFT)
			{
				int i = 0;
				while (i<5 && drakesGUID[i] != caster->GetGUID() && drakesGUID[i] != 0)
					i++;
				
				if (i == 5)
					return;
				
				drakesGUID[i] = caster->GetGUID();
				drakesDMG[i] = 0;
			}
		}
		...
		void DamageTaken(Unit* /*attacker*/, uint32& damage)
		{
			for (int i = 0; i < 5; i++)
			{
				...
				if (Aura* boost = me->GetAura(SPELL_TEMPORAL_RIFT, drakesGUID[i]))
				{
					drakesDMG[i] += damage;
					while(drakesDMG[i] > 15000)
					{
						if (Unit* drake = Unit::GetUnit(*me, drakesGUID[i]))
						{
							drake->CastSpell(me, SPELL_SHOCK_CHARGE, true);
							drakesDMG[i] -= 15000;
						}
					}
				}
		...
		{
			if (me->getVictim())
				if (me->getVictim()->GetDistance(me) > 80.0f)
				{
					EnterEvadeMode();
					return;
				}
			...
			{
				DoCastVictim(me->GetEntry() == NPC_DRAKE ? (IsHeroic() ? SPELL_ICE_BEAM_H : SPELL_ICE_BEAM_N) : (IsHeroic() ? SPELL_ARCANE_BOLT_H : SPELL_ARCANE_BOLT_N));
				
				abilityTimer = me->GetEntry() == NPC_DRAKE ? urand(9000, 14000) : urand(7000, 11000);
			}
			else
				abilityTimer -= diff;
		}
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_drakes_and_whelpsAI(creature);
    }
};
...
void AddSC_oculus()
{
	...
	new spell_shock_lance();
	new npc_drakes_and_whelps();
}
