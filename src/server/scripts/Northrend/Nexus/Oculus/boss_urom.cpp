...
public:
    ...
    struct boss_uromAI : public BossAI
    {
		...
		void SendDataToNext(uint32 group1, uint32 group2, uint32 group3)
		{
			group[0] = group1;
			group[1] = group2;
			group[2] = group3;
        ...
        void SpellHit(Unit* /*pCaster*/, const SpellInfo* pSpell)
        {
            switch (pSpell->Id)
            {
                case SPELL_SUMMON_MENAGERIE:
					...
					if (Creature* meNext = me->SummonCreature(me->GetEntry(), 968.66f, 1042.53f, 527.32f, 0.077f))
					{
						CAST_AI(boss_urom::boss_uromAI, meNext->AI())->SendDataToNext(group[0], group[1], group[2]);
                ...
                case SPELL_SUMMON_MENAGERIE_2:
					//LeaveCombat();
					me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
					if (Creature* meNext = me->SummonCreature(me->GetEntry(), 1164.02f, 1170.85f, 527.321f, 3.66f))
					{
						CAST_AI(boss_urom::boss_uromAI, meNext->AI())->SendDataToNext(group[0], group[1], group[2]);
						me->DespawnOrUnsummon(1);
					}
                ...
                case SPELL_SUMMON_MENAGERIE_3:
					//LeaveCombat();
					me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
					if (Creature* meNext = me->SummonCreature(me->GetEntry(), 1118.31f, 1080.377f, 508.361f, 4.25f))
					{
						CAST_AI(boss_urom::boss_uromAI, meNext->AI())->SendDataToNext(group[0], group[1], group[2]);
						me->DespawnOrUnsummon(1);
					}
