	...
	struct boss_malygosAI : public BossAI
	{
		...
		void SetData(uint32 data, uint32 value)
		{
			if (data == DATA_SUMMON_DEATHS && _phase == PHASE_TWO)
			{
				...
				if (_summonDeaths >= RAID_MODE(6,12))
					StartPhaseThree();
			}
		}
		...
		void StartPhaseThree()
		{
		  ...
		  //  me->SetInCombatWithZone();
			...
			//events.ScheduleEvent(EVENT_CHECK_PLAYERS,8*IN_MILLISECONDS);
		...
		void PrepareForVortex()
		{
			...
			//Position pos;
			//pos.Relocate(MalygosPositions[1].GetPositionX(), MalygosPositions[1].GetPositionY(), MalygosPositions[1].GetPositionZ());
			//me->GetMotionMaster()->MoveTakeoff(MOVE_VORTEX, pos, 10.0f);
		...
		void StartPhaseTwo()
		{
			...
			for (uint8 i = 0; i < RAID_MODE(4, 8); i++)
			{
				...
				uint8 pos = urand(0,1) ? MAX_HOVER_DISK_WAYPOINTS-1 : 0;
				if (Creature* summon = me->SummonCreature(NPC_HOVER_DISK_CASTER, HoverDiskWaypoints[pos]))
					if (summon->IsAIEnabled)
					{
						if (Creature* passenger = summon->SummonCreature(NPC_SCION_OF_ETERNITY, HoverDiskWaypoints[pos], TEMPSUMMON_CORPSE_DESPAWN, 1000))
						{
							passenger->EnterVehicle(summon);
							passenger->SetInCombatWithZone();
						}
						summon->AI()->DoAction(ACTION_HOVER_DISK_START_WP_1+urand(0,1));
					}
			}
			...
			for (uint8 i = 0; i < RAID_MODE(2, 4); i++)
			{
				...
				{
					if (Creature* passenger = summon->SummonCreature(NPC_NEXUS_LORD, HoverDiskWaypoints[0], TEMPSUMMON_CORPSE_DESPAWN, 1000))
					{
						passenger->EnterVehicle(summon);
						passenger->SetInCombatWithZone();
					}
			...
			void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
			{
				if (Unit* caster = GetCaster())
				{
					...
					if (Creature* malygos = caster->ToCreature())
					{
						...
						malygos->RemoveByteFlag(UNIT_FIELD_BYTES_1, 3, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER);
		...
		void PassengerBoarded(Unit* unit, int8 /*seat*/, bool apply)
		{
			...
			else
			{
				...
				if (unit->GetTypeId() == TYPEID_UNIT)
				{
					...
					{
						if (Creature* malygos = Unit::GetCreature(*me, _instance->GetData64(DATA_MALYGOS)))
							malygos->AI()->SetData(DATA_SUMMON_DEATHS, 0);
					}
