...
enum eEnums
{
    ...
    SPELL_TWILIGHT_TORMENT_VESP                 = 57935,    // (Shadow only) trigger 57935 then 57988
	...
	GUID_LAVA_STRIKE                            = 5134,
	ACTION_ACOLYTE_DIED                         = 2483,
	ACTION_CALL_DRAGON                          = 712,
	NPC_TWILIGHT_PORTAL                         = 2000202,

	ACHIEVEMENT_GONNA_GO_N                      = 2047,
	ACHIEVEMENT_GONNA_GO_H                      = 2048,
};
...
Location PortalPosition[4] = 
{
	{3247.3f, 533.1f, 59.0f},
	{3353.2f, 518.6f, 99.0f},
	{3250.4f, 657.1f, 87.0f},
	{3138.6f, 523.8f, 89.2f},
};
...
public:
    ...
    struct boss_sartharionAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (instance)
            {
                ...
                if (pTenebron)
                {
                    ...
                    {
                        if (instance->GetData(TYPE_TENEBRON_PREKILLED) == FAIL)
                        {
                            pTenebron->Respawn();
                            pTenebron->GetMotionMaster()->MoveTargetedHome();
                ...
                if (pShadron)
                {
                    ...
                    {
                        if (instance->GetData(TYPE_SHADRON_PREKILLED) == FAIL)
                        {
                            pShadron->Respawn();
                            pShadron->GetMotionMaster()->MoveTargetedHome();
                ...
                if (pVesperon)
                {
                    ...
                    {
                        if (instance->GetData(TYPE_VESPERON_PREKILLED) == FAIL)
                        {
                            pVesperon->Respawn();
                            pVesperon->GetMotionMaster()->MoveTargetedHome();
                ...
                if (map->IsDungeon())
                {
                    ...
                    if (!PlayerList.isEmpty())
                        for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
							lavaStrike.insert(i->getSource()->GetGUID());
                }
		...
		{
			if (type == GUID_LAVA_STRIKE)
				lavaStrike.erase(guid);
		}
        ...
        void JustDied(Unit* /*killer*/)
        {
			...
			for (std::set<uint64>::iterator ite = lavaStrike.begin(); ite != lavaStrike.end(); ite++)
			{
				if (Player* player = Unit::GetPlayer(*me, *ite))
					player->CompletedAchievement(sAchievementStore.LookupEntry(Is25ManRaid() ? ACHIEVEMENT_GONNA_GO_H : ACHIEVEMENT_GONNA_GO_N));
			}
        }
        ...
        void FetchDragons()
        {
            ...
            if (pFetchTene && pFetchTene->isAlive() && !pFetchTene->getVictim())
			{
				...
				if (instance->GetData(TYPE_TENEBRON_PREKILLED) == SPECIAL)
					instance->SetData(TYPE_TENEBRON_PREKILLED, FAIL);
            ...
            if (pFetchShad && pFetchShad->isAlive() && !pFetchShad->getVictim())
			{
				...
				if (instance->GetData(TYPE_SHADRON_PREKILLED) == SPECIAL)
					instance->SetData(TYPE_SHADRON_PREKILLED, FAIL);
            ...
            if (pFetchVesp && pFetchVesp->isAlive() && !pFetchVesp->getVictim())
			{
				...
				if (instance->GetData(TYPE_VESPERON_PREKILLED) == SPECIAL)
					instance->SetData(TYPE_VESPERON_PREKILLED, FAIL);
            ...
            if (instance)
            {
                if (Creature* temp = Unit::GetCreature(*me, instance->GetData64(uiDataId)))
                {
                    if (temp->isAlive() && !temp->getVictim())
                    {
						...
						temp->AI()->DoAction(ACTION_CALL_DRAGON);
		...
		bool isOutside(Player* player)
		{
			float x = player->GetPositionX();
			float y = player->GetPositionY();
			return x < 3218.86f || x > 3275.69f || y > 572.40f || y < 484.68f;
		}
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (m_uiFlameTsunamiTimer <= uiDiff)
            {
                ...
                switch (urand(0, 1))
                {
                    case 0:
                    {
                        Creature* Right1 = me->SummonCreature(NPC_FLAME_TSUNAMI, FlameRight1Spawn.x, FlameRight1Spawn.y, FlameRight1Spawn.z, 0);
                        Creature* Right2 = me->SummonCreature(NPC_FLAME_TSUNAMI, FlameRight2Spawn.x, FlameRight2Spawn.y, FlameRight2Spawn.z, 0);
                        Creature* Right3 = me->SummonCreature(NPC_FLAME_TSUNAMI, FlameRight3Spawn.x, FlameRight3Spawn.y, FlameRight3Spawn.z, 0);
                        Right1->GetMotionMaster()->MoveJump(FlameRight1Direction.x, FlameRight1Direction.y, FlameRight1Direction.z, 8.0f, 0.1f, 0);
                        Right2->GetMotionMaster()->MoveJump(FlameRight2Direction.x, FlameRight2Direction.y, FlameRight2Direction.z, 8.0f, 0.1f, 0);
                        Right3->GetMotionMaster()->MoveJump(FlameRight3Direction.x, FlameRight3Direction.y, FlameRight3Direction.z, 8.0f, 0.1f, 0);
                        break;
                    }
                    case 1:
                    {
                        Creature* Left1 = me->SummonCreature(NPC_FLAME_TSUNAMI, FlameLeft1Spawn.x, FlameLeft1Spawn.y, FlameLeft1Spawn.z, 0);
                        Creature* Left2 = me->SummonCreature(NPC_FLAME_TSUNAMI, FlameLeft2Spawn.x, FlameLeft2Spawn.y, FlameLeft2Spawn.z, 0);
                        Left1->GetMotionMaster()->MoveJump(FlameLeft1Direction.x, FlameLeft1Direction.y, FlameLeft1Direction.z, 8.0f, 0.1f, 0);
                        Left2->GetMotionMaster()->MoveJump(FlameLeft2Direction.x, FlameLeft2Direction.y, FlameLeft2Direction.z, 8.0f, 0.1f, 0);
                        break;
                    }
                }
...
struct dummy_dragonAI : public ScriptedAI
{
    ...
    void MovementInform(uint32 uiType, uint32 uiPointId)
    {
		...
		if (uiPointId == POINT_ID_INIT)
			m_uiWaypointId += urand(0, MAX_WAYPOINT);
		
		m_uiMoveNextTimer = 500;
        m_uiWaypointId = (m_uiWaypointId+1) % (MAX_WAYPOINT - 1);
    }
	...
	void ClearPortals()
	{
		if (acolyte)
		{
			acolyte->DespawnOrUnsummon(100);
			acolyte = NULL;
		}
		if (portal)
		{
			portal->DespawnOrUnsummon(100);
			portal = NULL;
		}
		...
		if (map->IsDungeon())
		{
			...
			for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
				if (i->getSource()->isAlive() && i->getSource()->HasAura(SPELL_TWILIGHT_SHIFT))
    ...
    void OpenPortal()
    {
        ...
        switch (me->GetEntry())
        {
            case NPC_TENEBRON:
            {
                ...
                {
					portal = me->SummonCreature(NPC_TWILIGHT_PORTAL, PortalPosition[2].x, PortalPosition[2].y, PortalPosition[2].z, 0);
                ...
                {
					portal = me->SummonCreature(NPC_TWILIGHT_PORTAL, PortalPosition[0].x, PortalPosition[0].y, PortalPosition[0].z, 0);
                    for (uint32 i = 0; i < 6; ++i)
            ...
            case NPC_SHADRON:
            {
                ...
                if (instance && instance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS)
				{
					portal = me->SummonCreature(NPC_TWILIGHT_PORTAL, PortalPosition[1].x, PortalPosition[1].y, PortalPosition[1].z, 0);
                    acolyte = me->SummonCreature(NPC_ACOLYTE_OF_SHADRON, AcolyteofShadron.x, AcolyteofShadron.y, AcolyteofShadron.z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 28000);
				}
                else
				{
					portal = me->SummonCreature(NPC_TWILIGHT_PORTAL, PortalPosition[0].x, PortalPosition[0].y, PortalPosition[0].z, 0);
                    acolyte = me->SummonCreature(NPC_ACOLYTE_OF_SHADRON, AcolyteofShadron2.x, AcolyteofShadron2.y, AcolyteofShadron2.z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 28000);
				}
            ...
            case NPC_VESPERON:
            {
                ...
                if (instance && instance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS)
				{
					portal = me->SummonCreature(NPC_TWILIGHT_PORTAL, PortalPosition[3].x, PortalPosition[3].y, PortalPosition[3].z, 0);
                    acolyte = me->SummonCreature(NPC_ACOLYTE_OF_VESPERON, AcolyteofVesperon.x, AcolyteofVesperon.y, AcolyteofVesperon.z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000);
				}
                else
				{
					portal = me->SummonCreature(NPC_TWILIGHT_PORTAL, PortalPosition[0].x, PortalPosition[0].y, PortalPosition[0].z, 0);
					acolyte = me->SummonCreature(NPC_ACOLYTE_OF_VESPERON, AcolyteofVesperon2.x, AcolyteofVesperon2.y, AcolyteofVesperon2.z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000);
				}
...
public:
    ...
    struct mob_tenebronAI : public dummy_dragonAI
    {
		...
		void DoAction(const int32 action)
		{
			dummy_dragonAI::DoAction(action);
			if (action == ACTION_ACOLYTE_DIED)
			{
				killedWhelps++;
				if (killedWhelps >= 6)
				{
    ...
    struct mob_vesperonAI : public dummy_dragonAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
			...
			if (!m_bHasPortalOpen)
			{
				...
				{
					m_bHasPortalOpen = true;
					DoCastAOE(SPELL_TWILIGHT_TORMENT_VESP);
					OpenPortal();
					m_uiAcolyteVesperonTimer = 30000;
				}
				else
        ...
        void JustDied(Unit* /*killer*/)
        {
            if (instance)
                if (Creature* Shadron = instance->instance->GetCreature(instance->GetData64(DATA_SHADRON)))
					(CAST_AI(mob_shadron::mob_shadronAI, Shadron->AI()))->DoAction(ACTION_ACOLYTE_DIED);
        }
    };
...
public:
    ...
    struct mob_acolyte_of_vesperonAI : public ScriptedAI
    {
        ...
        {
            if (instance)
                if (Creature* pVesperon = instance->instance->GetCreature(instance->GetData64(DATA_VESPERON)))
					(CAST_AI(mob_vesperon::mob_vesperonAI, pVesperon->AI()))->DoAction(ACTION_ACOLYTE_DIED);
        }
    };
...
public:
    ...
    struct mob_twilight_eggsAI : public NullCreatureAI
    {
        mob_twilight_eggsAI(Creature* creature) : NullCreatureAI(creature)
        {
            instance = creature->GetInstanceScript();
        }
        ...
        void SpawnWhelps()
        {
            ...
            if (!instance->GetData(TYPE_SARTHARION_EVENT) == IN_PROGRESS)
				me->SummonCreature(NPC_TWILIGHT_WHELP, *me, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000)->SetPhaseMask(1, true);
            else
                me->SummonCreature(NPC_SHARTHARION_TWILIGHT_WHELP, *me, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000)->SetPhaseMask(1, true);
		...
		void JustDied(Unit* killer)
		{
			if (Creature* Tenebron = instance->instance->GetCreature(instance->GetData64(DATA_TENEBRON)))
				(CAST_AI(mob_tenebron::mob_tenebronAI, Tenebron->AI()))->DoAction(ACTION_ACOLYTE_DIED);
		}
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_fire_cycloneAI(creature);
    }

    struct npc_fire_cycloneAI : public NullCreatureAI
    {
		npc_fire_cycloneAI(Creature* creature) : NullCreatureAI(creature) { }

		void SpellHitTarget(Unit* target, const SpellInfo* spell)
		{
			if (target->ToPlayer())
				if (InstanceScript* instance = me->GetInstanceScript())
					if (Creature* sartharion = me->GetCreature(*me, instance->GetData64(DATA_SARTHARION)))
						sartharion->AI()->SetGUID(target->GetGUID(), GUID_LAVA_STRIKE);
		}
...
class npc_sartharion_portal : public CreatureScript
{
public:
    npc_sartharion_portal() : CreatureScript("npc_sartharion_portal") { }

	bool OnGossipHello(Player* player, Creature* portal)
	{
		player->CastSpell(player, 57620);
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_sartharion_portalAI(creature);
    }

    struct npc_sartharion_portalAI : public NullCreatureAI
    {
		npc_sartharion_portalAI(Creature* creature) : NullCreatureAI(creature)
		{
			me->setFaction(35);
			me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
		}
