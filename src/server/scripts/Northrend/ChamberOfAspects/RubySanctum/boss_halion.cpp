					...
					case DATA_FIGHT_PHASE:
						...
						switch (value)
						{
							...
							case PHASE_TWO:
								...
								_events.ScheduleEvent(EVENT_SHADOW_PULSARS_WARN, 24000);
								break;
							case PHASE_THREE:
								_events.ScheduleEvent(EVENT_CHECK_CORPOREALITY, 5000);
	...
	public:
		...
		struct npc_orb_carrierAI : public ScriptedAI
		{
				...
				instance(creature->GetInstanceScript())
			{
				...
				Creature* orb;
				orb = me->SummonCreature(NPC_SHADOW_ORB_N, HalionSpawnPos);
				//orb->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC);
				orbs[0] = orb->GetGUID();
				
				orb = me->SummonCreature(NPC_SHADOW_ORB_S, HalionSpawnPos);
				orbs[1] = orb->GetGUID();
				
				if (!IsHeroic())
					return;
				
				orb = me->SummonCreature(NPC_SHADOW_ORB_E, HalionSpawnPos);
				orbs[2] = orb->GetGUID();
				
				orb = me->SummonCreature(NPC_SHADOW_ORB_W, HalionSpawnPos);
				orbs[3] = orb->GetGUID();
			}
			...
			void JustDied(Unit* /*killer*/)
			{
				...
				{
					Creature* orb = ObjectAccessor::GetCreature(*me, orbs[i]);
			...
			void UpdateAI(uint32 const diff)
			{
				...
				{
					Position pos;
					Unit* orb = ObjectAccessor::GetUnit(*me, orbs[i]);
					float ang = me->GetOrientation() - i * static_cast<float>(M_PI);
					MapManager::NormalizeOrientation(ang);
					me->GetNearPosition(pos, 46.0f, ang);
					orb->NearTeleportTo(pos.GetPositionX(), pos.GetPositionY(), me->GetPositionZ(), ang);
				}
				...
				if (IsHeroic())
				{
					for (uint8 i = 2; i < 4; i++)
					{
						Position pos;
						Unit* orb = ObjectAccessor::GetUnit(*me, orbs[i]);
						float ang = me->GetOrientation() - (i-2) * static_cast<float>(M_PI) + static_cast<float>(M_PI/2);
						MapManager::NormalizeOrientation(ang);
						me->GetNearPosition(pos, 46.0f, ang);
						orb->NearTeleportTo(pos.GetPositionX(), pos.GetPositionY(), me->GetPositionZ(), ang);
					}
				...
				if (isCutting)
				{
					if (isCutting % 300 < diff)
					{
						//Cast Cutter
						for (uint8 i = 0; i < 4; i++)
						{
							Unit* target = ObjectAccessor::GetUnit(*me, orbs[i]);
							me->CastSpell(target, SPELL_TWILIGHT_CUTTER_TRIGGERED, true);
						}
			...
			void DoAction(int32 const action)
			{
				if (action == ACTION_SHOOT)
				{
					...
					*/
					Unit* northOrb = ObjectAccessor::GetUnit(*me, orbs[0]);
					Unit* southOrb = ObjectAccessor::GetUnit(*me, orbs[1]);
					if (southOrb && northOrb)
					{
						if (northOrb->GetTypeId() == TYPEID_UNIT)
							northOrb->ToCreature()->AI()->Talk(EMOTE_WARN_LASER);
					...
					Unit* eastOrb = ObjectAccessor::GetUnit(*me, orbs[2]);
					Unit* westOrb = ObjectAccessor::GetUnit(*me, orbs[3]);
					if (eastOrb && westOrb)
						TriggerCutter(eastOrb, westOrb);
				}
			}
	...
	public:
		...
		struct npc_combustion_consumptionAI : public Scripted_NoMovementAI
		{
			...
			void SetData(uint32 type, uint32 stackAmount)
			{
				...
				me->CastCustomSpell(_explosionSpell, SPELLVALUE_BASE_POINT0, damage, summoner);
			}
	...
	public:
		...
		public:
			...
			void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
			{
				...
				SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(_summonSpellId);
				uint32 entry = uint32(spellInfo->Effects[0].MiscValue);
				SummonPropertiesEntry const* properties = sSummonPropertiesStore.LookupEntry(uint32(spellInfo->Effects[0].MiscValueB));
				uint32 duration = uint32(spellInfo->GetDuration());
				Position pos;
				GetTarget()->GetPosition(&pos);
				
				if (Creature* summon = GetTarget()->GetMap()->SummonCreature(entry, pos, properties, duration, GetTarget(), spellInfo->Id))
					if (summon->IsAIEnabled)
						summon->AI()->SetData(DATA_STACKS_DISPELLED, aurEff->GetBase()->GetStackAmount());
...
class spell_halion_twilight_cutter_trigger : public SpellScriptLoader
{
	public:
		spell_halion_twilight_cutter_trigger() : SpellScriptLoader("spell_halion_twilight_cutter_trigger") { }

		class spell_halion_twilight_cutter_trigger_AuraScript : public AuraScript
		{
			PrepareAuraScript(spell_halion_twilight_cutter_trigger_AuraScript);
		...
		AuraScript* GetAuraScript() const
		{
			return new spell_halion_twilight_cutter_trigger_AuraScript();
		}
};
...
void AddSC_boss_halion()
{
	...
	new spell_halion_twilight_cutter_trigger();
}
