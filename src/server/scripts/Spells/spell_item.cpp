...
class spell_item_unusual_compass : public SpellScriptLoader
{
    public:
        ...
        class spell_item_unusual_compass_SpellScript : public SpellScript
        {
            ...
            {
                Unit* caster = GetCaster();
				float random = frand(0.0f, 62832.0f) / 10000.0f;
                caster->SetOrientation(random);
				caster->SetFacingTo(random);
...
enum Torches
{
   SPELL_TORCH_THROW_VISUAL = 45276,                           // JUGGLING TORCH CREDIT SPELL
   SPELL_TORCH_THROW = 45819,                                  // JUGGLING TORCH ITEM SPELL
   SPELL_BRAZIER_HIT_CREDIT = 45724,                           // CREDIT 4 Q TORCH TOSSING
   SPELL_TORCH_CATCH_CREDIT = 43300,                           // CREDIT 4 Q TORCH CATCHING
   SPELL_PRACTICE_TORCH = 46054,                               // PRACTICE TORCH ITEM SPELL
   SPELL_ITEM_UNLIT_TORCH = 46747,                             // UNLIT TORCHES ITEM SPELL
   SPELL_TORCH_THROW_RANDOM = 45669,                           // 4 Q TORCH CATCHING
   SPELL_TORCH_CATCH = 45671,                                  // SPELL TORCH THROW RANDOM ONHIT SPELL
   NPC_INVISIBLE_TRIGGER = 48381,
   GO_TORCH_BRAZIER = 187708,
   QUEST_TORCH_TOSSING_ALI = 11731,
   QUEST_TORCH_TOSSING_HORDE = 11922,
   QUEST_TORCH_CATCHING_ALI = 11657,
   QUEST_TORCH_CATCHING_HORDE = 11923,
   QUEST_TORCH_TOSSING_DAILY_ALI = 11921,
   QUEST_TORCH_TOSSING_DAILY_HORDE = 11926,
   QUEST_TORCH_CATCHING_DAILY_ALI = 11924,
   QUEST_TORCH_CATCHING_DAILY_HORDE = 11925,
   SPELL_TORCH_TOSSING_COMPLETE_CREDIT_ALI = 45719,
   SPELL_TORCH_TOSSING_COMPLETE_CREDIT_HORDE = 46651,
   SPELL_TORCH_CATCHING_COMPLETE_CREDIT_ALI = 46081,
   SPELL_TORCH_CATCHING_COMPLETE_CREDIT_HORDE = 46654,
};

class spell_item_juggling_torch : public SpellScriptLoader
{
   public:
       spell_item_juggling_torch() : SpellScriptLoader("spell_item_juggling_torch") {}

       class spell_item_juggling_torch_SpellScript : public SpellScript
       {
           PrepareSpellScript(spell_item_juggling_torch_SpellScript)
           bool Validate(SpellInfo const* /*spellEntry*/)
           {
               if (!sSpellMgr->GetSpellInfo(SPELL_TORCH_THROW))
                   return false;
               return true;
           }
           ...
           void HandleScript(SpellEffIndex /*effIndex*/)
           {
               ...
               if (const Position* pos = GetExplTargetDest())
                   if (TempSummon* summon = caster->SummonCreature(NPC_INVISIBLE_TRIGGER, pos->GetPositionX(), pos->GetPositionY(), pos->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 10000))
                       caster->CastSpell(summon, SPELL_TORCH_THROW_VISUAL, true);
           }

           void Register()
           {
               OnEffectLaunch += SpellEffectFn(spell_item_juggling_torch_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_DUMMY);
           }
       ...
       SpellScript* GetSpellScript() const
       {
           return new spell_item_juggling_torch_SpellScript();
       }
};

class spell_juggling_torch_visual : public SpellScriptLoader
{
   public:
       spell_juggling_torch_visual() : SpellScriptLoader("spell_juggling_torch_visual") {}

       class spell_juggling_torch_visual_SpellScript : public SpellScript
       {
           PrepareSpellScript(spell_juggling_torch_visual_SpellScript)
           bool Validate(SpellInfo const* /*spellEntry*/)
           {
               if (!sSpellMgr->GetSpellInfo(SPELL_TORCH_THROW_VISUAL))
                   return false;
               return true;
           }
           ...
           void Trigger()
           {
               ...
               if (target && target->GetTypeId() == TYPEID_UNIT)
                   if (caster->IsWithinDistInMap(target,3.0f,false))
                       if (player->GetQuestStatus(QUEST_TORCH_CATCHING_ALI) == QUEST_STATUS_REWARDED || player->GetQuestStatus(QUEST_TORCH_CATCHING_HORDE) == QUEST_STATUS_REWARDED)
                           caster->CastSpell(caster, SPELL_TORCH_THROW_VISUAL,true);
           }

           void Register()
           {
               OnHit += SpellHitFn(spell_juggling_torch_visual_SpellScript::Trigger);
           }
       ...
       SpellScript* GetSpellScript() const
       {
           return new spell_juggling_torch_visual_SpellScript();
       }
};

class spell_item_unlit_torches : public SpellScriptLoader
{
   public:
       spell_item_unlit_torches() : SpellScriptLoader("spell_item_unlit_torches") {}

       class spell_item_unlit_torches_SpellScript : public SpellScript
       {
           PrepareSpellScript(spell_item_unlit_torches_SpellScript)
           bool Validate(SpellInfo const* /*spellEntry*/)
           {
               if (!sSpellMgr->GetSpellInfo(SPELL_ITEM_UNLIT_TORCH))
                   return false;
               return true;
           }

           SpellCastResult OnCast()
           {
               Unit* caster = GetCaster();
               float destX, destY, destZ;

               destX = float(irand(-8,8)) + caster->GetPositionX();
               destY = float(irand(-8,8)) + caster->GetPositionY();
               destZ = caster->GetPositionZ();
               caster->RemoveAurasDueToSpell(SPELL_TORCH_CATCH_CREDIT);
               caster->CastSpell(destX, destY, destZ, SPELL_TORCH_THROW_RANDOM, true);
               return SPELL_CAST_OK;
           }

           void Register()
           {
               OnCheckCast += SpellCheckCastFn(spell_item_unlit_torches_SpellScript::OnCast);
           }
       };

       SpellScript* GetSpellScript() const
       {
           return new spell_item_unlit_torches_SpellScript();
       }
};

class spell_torch_catch : public SpellScriptLoader
{
   public:
       spell_torch_catch() : SpellScriptLoader("spell_torch_catch") {}

       class spell_torch_catch_SpellScript : public SpellScript
       {
           PrepareSpellScript(spell_torch_catch_SpellScript)
           bool Validate(SpellInfo const* /*spellEntry*/)
           {
               if (!sSpellMgr->GetSpellInfo(SPELL_TORCH_CATCH))
                   return false;
               return true;
           }
           ...
           void Trigger()
           {
               ...
               if (player->GetQuestStatus(QUEST_TORCH_CATCHING_ALI) == QUEST_STATUS_INCOMPLETE ||
                   player->GetQuestStatus(QUEST_TORCH_CATCHING_HORDE) == QUEST_STATUS_INCOMPLETE ||
                   player->GetQuestStatus(QUEST_TORCH_CATCHING_DAILY_ALI) == QUEST_STATUS_INCOMPLETE ||
                   player->GetQuestStatus(QUEST_TORCH_CATCHING_DAILY_HORDE) == QUEST_STATUS_INCOMPLETE)
                   {
                       caster->AddAura(SPELL_TORCH_CATCH_CREDIT, caster);
                       if (caster->GetAuraCount(SPELL_TORCH_CATCH_CREDIT) >= 4)
                       {
                           caster->CastSpell(caster, SPELL_TORCH_CATCHING_COMPLETE_CREDIT_ALI, true);
                           caster->CastSpell(caster, SPELL_TORCH_CATCHING_COMPLETE_CREDIT_HORDE, true);
                           caster->RemoveAurasDueToSpell(SPELL_TORCH_CATCH_CREDIT);
                       }
                   }
           }
           ...
           void Register()
           {
               OnHit += SpellHitFn(spell_torch_catch_SpellScript::Trigger);
           }
       ...
       SpellScript* GetSpellScript() const
       {
           return new spell_torch_catch_SpellScript();
       }
};
...
void AddSC_item_spell_scripts()
{
	...
	new spell_item_juggling_torch();
    new spell_juggling_torch_visual();
    new spell_item_unlit_torches();
    new spell_torch_catch();
}
