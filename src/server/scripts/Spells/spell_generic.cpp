...
class spell_gen_lifeblood : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_lifeblood_AuraScript : public AuraScript
        {
            ...
            {
                if (Unit* owner = GetUnitOwner())
                    amount += int32(CalculatePctN(owner->GetMaxHealth(), 1.5f / aurEff->GetTotalTicks()));
            }
        ...
        void HandleEffectPeriodic(AuraEffect const * aurEff)
        {
            if (Unit* caster = GetCaster())
            {
                if (caster->GetOwner())
                {
                    ...
                    {
                        caster->Mount(29736);
                        caster->SetSpeed(MOVE_WALK, 1.9f);
                        caster->SetSpeed(MOVE_RUN, 1.9f);
                    }
                    else if (caster->IsMounted())
                    {
                        caster->Dismount();
                        caster->SetSpeed(MOVE_WALK, 0.95f);
                        caster->SetSpeed(MOVE_RUN, 0.95f);
                    }
                    else
                    {
                        caster->SetSpeed(MOVE_WALK, 0.95f);
